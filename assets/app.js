...
async function getUsableSession({forceRefresh=false}={}){
  ...
  let currentSession=getData?.session||null;
...
async function refreshAppData(){
  ...
  setSync('Refreshing session…');

  try{
    await withStartupTimeout(getUsableSession({forceRefresh:true}),'Session refresh',10000);
    setSync('Connecting…');
    await bootstrapApp();

    if(assets.length || plantRooms.length || valveRecords.length){
      setSync(`Cloud synced · ${assets.length} assets`);
    }else{
      setSync('Connected · 0 records returned',true);
    }
  }catch(error){
...
async function bootstrapApp(){
  ...
  try{
    await withStartupTimeout(getUsableSession(),'Session validation',10000);
...
function stopApp() { session=null; appStarting=false; appBootstrapping=false; assets=[]; els.appShell.hidden=true; els.authScreen.hidden=false; els.authPassword.value=''; els.authMessage.textContent=''; }

els.signIn.onclick=signIn; els.signUp.onclick=signUp; els.signOut.onclick=()=>client.auth.signOut(); els.refresh.onclick=refreshAppData;
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('/service-worker.js?v=9990012',{updateViaCache:'none'});
      await reg.update();
