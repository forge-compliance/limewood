...
async function bootstrapApp(){
  ...
  try{
    await withStartupTimeout(loadCloud(),'Initial cloud sync');
    cloudReady=true;
  }catch(error){
    console.error('Initial cloud sync failed:',error);
    setSync(`Signed in · cloud sync issue: ${error?.message||error}`,true);
  }

  // Only run the one-time seed when the basic cloud tables are reachable.
  if(cloudReady){
    try{
      await withStartupTimeout(seedExistingAssets(),'Asset seed');
      await withStartupTimeout(loadCloud(),'Cloud refresh');
    }catch(error){
  ...
  try{ refreshV6Metrics(); }catch(error){ console.warn('Metric refresh failed',error); }

  // These modules are useful but must never block access to the dashboard.
  const optionalLoads = [
    ['Document Centre', loadDocumentCentre],
    ['Operations', loadOperations],
    ['Logs', loadLogs]
  ];

  const results = await Promise.allSettled(
    optionalLoads.map(([label,fn])=>withStartupTimeout(Promise.resolve().then(fn),label,15000))
  );

  results.forEach((result,i)=>{
    if(result.status==='rejected') console.warn(`${optionalLoads[i][0]} startup failed:`,result.reason);
  });
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('/service-worker.js?v=9990010',{updateViaCache:'none'});
      await reg.update();
