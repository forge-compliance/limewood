// ===== Limewood Creator bridge =====
// Editing controls are exposed ONLY inside the Creator shell.
// The normal engineering website can still read and render log templates,
// but it cannot open the Log Builder.
const LIMEWOOD_CREATOR_MODE =
  new URLSearchParams(window.location.search).get('creator') === '1' ||
  /LimewoodCreatorAndroid/i.test(navigator.userAgent || '');

if (LIMEWOOD_CREATOR_MODE) {
  window.LIMEWOOD_CREATOR = {
    version: '1.3.0',
    ready: false,
    openLogBuilder: function () {
      window.dispatchEvent(new CustomEvent('limewood:creator:open-log-builder'));
      return true;
    },
    openLocationBuilder: function () {
      window.dispatchEvent(new CustomEvent('limewood:creator:open-location-builder'));
      return true;
    },
    reloadLogTemplates: function () {
      window.dispatchEvent(new CustomEvent('limewood:creator:reload-log-templates'));
      return true;
    }
  };
}
...
// Creator commands are received only when this page is running in Creator mode.
if (LIMEWOOD_CREATOR_MODE) {
  window.addEventListener('limewood:creator:open-log-builder', () => {
    Promise.resolve(openCreatorLogBuilder()).catch(error => {
      console.error('Creator Log Builder failed to open:', error);
      alert('Log Builder could not open: ' + (error?.message || error));
    });
  });

  window.addEventListener('limewood:creator:open-location-builder', () => {
    Promise.resolve(openCreatorLocationBuilder()).catch(error => {
      console.error('Creator Location Builder failed to open:', error);
      alert('Locations & Areas could not open: ' + (error?.message || error));
    });
  });

  window.addEventListener('limewood:creator:reload-log-templates', () => {
    Promise.resolve(loadLogTemplates()).catch(error => {
      console.error('Creator log templates failed to reload:', error);
    });
  });

  if (window.LIMEWOOD_CREATOR) window.LIMEWOOD_CREATOR.ready = true;
}

const cfg = window.LIMEWOOD_CONFIG || {};
const bmsCfg = window.LIMEWOOD_BMS || {
...
const staticBase = Array.isArray(window.LIMEWOOD_ASSETS)
  ? window.LIMEWOOD_ASSETS
  : [];

const staticByCode = new Map(
  staticBase.map(a => [a.id, a])
);
...
// Remove legacy Limewood application data from browser storage.
// Supabase auth session storage is left untouched.
try {
  [
    'limewood-v52-operations',
    'limewood-custom-plant-rooms-v1',
    'limewood-v64-log-entries',
    'limewood-v7-valve-migration-error',
    'limewood-v7-valves-migrated',
    'limewood-last-valve-import-room'
  ].forEach(k => localStorage.removeItem(k));

  for(let i=localStorage.length-1;i>=0;i--){
    const k=localStorage.key(i);

    if(k && k.startsWith('lw-')){
...
const fields = [
  ...
  ['mechanicalIsolation','Mechanical isolation'],
  ['isolationProcedure','Isolation procedure'],
  ['ppm','PPM frequency']
];
...
const els = {
  ...
  manufacturerLink:$('mManufacturerLink'),
  manualLink:$('mManualLink'),
  photos:$('mPhotos'),
  ...
  assetBmsBtn:$('assetBmsBtn'),
  assetQrBtn:$('assetQrBtn'),
  maintenanceHistoryBtn:$('maintenanceHistoryBtn'),
...
const docEls = {
  ...
  buildingFilter:$('documentBuildingFilter'),
  statusFilter:$('documentStatusFilter'),

  add:$('addDocumentBtn'),
  modal:$('documentModal'),
  close:$('closeDocumentModal'),
  save:$('saveDocumentBtn'),
  ...
  author:$('dAuthor'),
  approvedBy:$('dApprovedBy'),
  issueDate:$('dIssueDate'),
  reviewDate:$('dReviewDate'),
  building:$('dBuilding'),
  plantRoom:$('dPlantRoom'),
  asset:$('dAsset'),

  description:$('dDescription'),
  changeSummary:$('dChangeSummary'),
  file:$('dFile')
};
...
let plantRooms = [];
let locationAreas = [];
let locationSubAreas = [];
let locationTablesReady = false;

let current = null;
let visibleRows = [];
let editing = false;

// Register context used by Plant Room discipline views.
// This prevents a later render() from silently
// falling back to another room.
let registerForcedRoom = '';
let registerDisciplineTerms = [];
let registerDisciplineTitle = '';
...
function bmsUrl(key='overview') {
  return `${
    String(bmsCfg.baseUrl||'').replace(/\/$/,'')
  }${
    bmsRoutes[key] || bmsRoutes.overview
  }`;
}
...
function openBms(key='overview') {
  ...
  if(!url){
    return alert(
      'The BMS address has not been configured.'
    );
  }
...
function matchingComplianceDocs(register){
  return libraryDocuments.filter(d=>{
    ...
    return register.terms.some(
      t=>h.includes(t)
    );
  });
}

function complianceDue(d){
  const raw=d.raw||{};
...
function renderCompliance(){
  ...
  const evidence=$('complianceEvidenceList');

  if(!grid || !evidence) return;

  const allMatches=new Map();

  complianceRegisters.forEach(r=>
    matchingComplianceDocs(r).forEach(
      d=>allMatches.set(d.key,d)
    )
  );

  const docs=[...allMatches.values()];

  $('complianceDocCount').textContent=docs.length;

  $('complianceDueCount').textContent=
    docs.filter(complianceDue).length;

  $('isolationCount').textContent=
    assets.filter(
      a=>
        a.electricalIsolation ||
        a.mechanicalIsolation ||
        a.isolationProcedure
    ).length;

  $('qrAssetCount').textContent=assets.length;

  grid.innerHTML=
    complianceRegisters.map(r=>{
      const matches=matchingComplianceDocs(r);
      const due=matches.filter(complianceDue).length;

      return `
        <article class="complianceCard">
          <div class="complianceCardHead">
            <span>${r.icon}</span>
            ...
            <div>
              ...
              <small>
                ${matches.length}
                evidence record${matches.length===1?'':'s'}
              </small>
            </div>
          ...
          <div class="complianceCardActions">
            <button data-compliance-open="${esc(r.key)}">
              Open evidence
            </button>

            <button data-compliance-upload="${esc(r.category)}">
              + Upload
            </button>
          </div>
        </article>
  ...
  $('complianceEvidenceCount').textContent=
    `${docs.length} records`;
  ...
  evidence.innerHTML=
    latest.length
      ? latest.map(d=>`
          <article>
            <div>
              ...
              <small>
                ${esc(documentTypeLabel(d.type))}
                ${d.number ? ' · '+esc(d.number) : ''}
              </small>
            </div>
      ...
      : `
        <p class="emptyState">
          No compliance evidence has been classified yet.
          Upload evidence and use a clear category or title.
        </p>
      `;
}
...
function openComplianceUpload(category='Compliance'){
  showDocuments('');
  openDocumentUpload();

  docEls.category.value=category;
}
...
function showQrForAsset(asset){
  ...
  $('qrAssetMeta').textContent=
    `${
      asset.room
    } · ${
      asset.manufacturer || 'Manufacturer to confirm'
    } ${
      asset.model || ''
    }`.trim();
  ...
  if(typeof QRCode==='undefined'){
    box.textContent=
      'QR library could not load. Check the internet connection and reload.';
  } else {
    new QRCode(box,{
      text:assetUrl(asset.id),
      width:240,
      height:240,
...
function printQrAsset(asset){
  ...
  w.document.write(`
    <title>${esc(asset.id)} QR label</title>
...
function printAllQrLabels(){
  if(typeof QRCode==='undefined'){
    return alert(
      'QR library could not load. Reload while connected to the internet.'
    );
  }
  ...
  w.document.write(`
    <title>Lime Wood Asset QR Labels</title>
  ...
  const s=w.document.getElementById('sheet');
...
// Cloud-only operational data.
// No valves, PPM records or plant-room data
...
function frequencyMonths(freq=''){
  ...
  if(f.includes('2 year')) return 24;
...
async function loadOperations(){
  ...
  if(v.error){
    setSync(
      'Valve register unavailable - database connection failed',
      true
    );
  }
}
...
function canonicalPlantRoomName(name){
  const value=
    normalisePlantRoomName(name);

  const key=value.toLowerCase();

  if(
    key==='forest cottage plant room' ||
    key==='forest lodges plant room' ||
    key==='forest lodge plant room' ||
    key==='forest cottage & lodges plant room'
  ){
    return 'Forest Cottage & Lodges Plant Room';
  }

  // Historic typo created a second dead room card.
  // Treat both spellings as one room everywhere.
  return value;
...
let logTemplateCloudReady=false;

function normaliseLogTemplateRow(row){
  return {
...
async function loadLogTemplates(){
  try{
    ...
    logTemplateCloudReady=true;

    if((data||[]).length){
      const next={};
      for(const row of data){
        const t=normaliseLogTemplateRow(row);
        next[t.key]=t;
      }
      LOG_TEMPLATES=next;
    }
  }catch(error){
    logTemplateCloudReady=false;
    console.warn('Dynamic log templates unavailable. Using built-in templates.',error);
  }
...
async function loadLocationDirectory(){
  try{
    const [aRes,sRes]=await Promise.all([
      client.from('location_areas').select('*').order('sort_order',{ascending:true}).order('name'),
      client.from('location_sub_areas').select('*').order('sort_order',{ascending:true}).order('name')
    ]);
    if(aRes.error)throw aRes.error;
    if(sRes.error)throw sRes.error;
    locationAreas=aRes.data||[];
    locationSubAreas=sRes.data||[];
    locationTablesReady=true;
  }catch(error){
    locationAreas=[];
    locationSubAreas=[];
    locationTablesReady=false;
    console.warn('Locations & Areas tables unavailable; legacy plant rooms remain active.',error);
  }
}

function assetBuildingForName(name=''){
  return buildings.find(b=>b.name===name)||null;
}
function locationAreaForName(buildingId,name=''){
  return locationAreas.find(a=>String(a.building_id)===String(buildingId||'')&&a.name===name)||null;
}
function populateAssetLocationEditor(){
  const bSel=$('assetBuildingSelect'),aSel=$('assetAreaSelect'),sSel=$('assetSubAreaSelect');
  if(!bSel||!aSel||!sSel||!current)return;
  const legacyRoom=plantRooms.find(p=>p.name===current.room);
  const currentBuilding=assetBuildingForName(current.buildingName)||buildings.find(b=>String(b.id)===String(current.raw?.building_id||legacyRoom?.building_id||''));
  bSel.innerHTML='<option value="">Select building</option>'+buildings.map(b=>`<option value="${esc(b.id)}">${esc(b.name)}</option>`).join('');
  if(currentBuilding)bSel.value=currentBuilding.id;
  const renderAreas=()=>{
    const bid=bSel.value;
    const modern=locationAreas.filter(a=>String(a.building_id)===String(bid));
    const legacy=plantRooms.filter(p=>String(p.building_id)===String(bid)&&!modern.some(a=>a.name===p.name)).map(p=>({id:'plant:'+p.id,name:p.name,area_type:'Plant Room'}));
    const rows=[...modern,...legacy];
    aSel.innerHTML='<option value="">Select area / location</option>'+rows.map(a=>`<option value="${esc(a.id)}">${esc(a.name)}${a.area_type?` · ${esc(a.area_type)}`:''}</option>`).join('');
    const match=rows.find(a=>a.name===current.areaName||a.name===current.room);
    if(match)aSel.value=match.id;
    renderSubs();
  };
  const renderSubs=()=>{
    const aid=aSel.value;
    const rows=aid.startsWith('plant:')?[]:locationSubAreas.filter(s=>String(s.area_id)===String(aid));
    sSel.innerHTML='<option value="">No sub-area</option>'+rows.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('');
    const match=rows.find(s=>s.name===current.subAreaName);
    if(match)sSel.value=match.id;
  };
  bSel.onchange=()=>{current.buildingName=buildings.find(b=>b.id===bSel.value)?.name||'';current.areaName='';current.subAreaName='';renderAreas()};
  aSel.onchange=()=>{const val=aSel.value;const area=val.startsWith('plant:')?plantRooms.find(p=>('plant:'+p.id)===val):locationAreas.find(a=>String(a.id)===String(val));current.areaName=area?.name||'';current.room=current.areaName;current.areaId=val.startsWith('plant:')?null:(area?.id||null);current.areaType=val.startsWith('plant:')?'Plant Room':(area?.area_type||'');current.subAreaName='';renderSubs()};
  sSel.onchange=()=>{const sub=locationSubAreas.find(s=>String(s.id)===String(sSel.value));current.subAreaName=sub?.name||'';current.subAreaId=sub?.id||null};
  renderAreas();
}

async function resolveAssetLocation(asset){
  const bSel=$('assetBuildingSelect'),aSel=$('assetAreaSelect'),sSel=$('assetSubAreaSelect');
  const selectedBuildingId=bSel?.value||assetBuildingForName(asset.buildingName)?.id||null;
  const selectedArea=aSel?.value||asset.areaId||'';
  const selectedSub=sSel?.value||asset.subAreaId||null;
  if(selectedArea&&String(selectedArea).startsWith('plant:')){
    const pr=plantRooms.find(p=>('plant:'+p.id)===selectedArea);
    if(!pr)throw new Error('Selected plant room could not be found.');
    return {building_id:pr.building_id,plant_room_id:pr.id,area_id:null,sub_area_id:null};
  }
  const area=locationAreas.find(a=>String(a.id)===String(selectedArea));
  if(area)return {building_id:area.building_id,plant_room_id:null,area_id:area.id,sub_area_id:selectedSub||null};
  if(asset.room){
    const pr=plantRooms.find(p=>p.name===asset.room);
    if(pr)return {building_id:pr.building_id,plant_room_id:pr.id,area_id:null,sub_area_id:null};
  }
  if(!selectedBuildingId)throw new Error('Choose a building and area for this asset.');
  throw new Error('Choose an area / location for this asset.');
}

const CREATOR_AREA_TYPES=['Plant Room','Kitchen','Café','Restaurant / Bar','Guest Area','Bedroom','Office','Store','Service Cupboard','Riser','Roof','External','Spa / Leisure','Workshop','Back of House','Other'];

function ensureCreatorLocationBuilder(){
  let modal=$('creatorLocationBuilder');
  if(modal)return modal;
  const style=document.createElement('style');
  style.id='creatorLocationBuilderStyle';
  style.textContent=`
    #creatorLocationBuilder{position:fixed;inset:0;z-index:2147483646;background:rgba(8,24,20,.82);padding:18px;overflow:auto}#creatorLocationBuilder[hidden]{display:none!important}
    .locShell{max-width:1100px;margin:auto;background:#f6f2e9;border-radius:22px;overflow:hidden;color:#173d33;box-shadow:0 24px 80px rgba(0,0,0,.35)}
    .locHead{display:flex;justify-content:space-between;align-items:center;padding:18px 20px;background:#173d33;color:#fff}.locHead h2{margin:0}.locHead p{margin:4px 0 0;opacity:.8;font-size:13px}
    .locHead button,.locBtn{border:0;border-radius:11px;padding:10px 13px;font-weight:800}.locHead button{background:#f6f2e9;color:#173d33}
    .locBody{padding:18px}.locToolbar{display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px;margin-bottom:16px}.locToolbar input,.locToolbar select{padding:11px;border:1px solid rgba(23,61,51,.22);border-radius:10px;background:#fff}
    .locBtn{background:#173d33;color:#fff}.locBuilding{background:#fff;border:1px solid rgba(23,61,51,.15);border-radius:15px;margin:12px 0;overflow:hidden}.locBuildingHead{display:flex;justify-content:space-between;align-items:center;padding:13px 15px;background:#eee8db}.locBuildingHead h3{margin:0}.locArea{padding:12px 15px;border-top:1px solid rgba(23,61,51,.1)}.locAreaTop{display:flex;justify-content:space-between;gap:10px;align-items:center}.locArea small{opacity:.65}.locSubs{padding:8px 0 0 18px;font-size:13px}.locActions{display:flex;gap:6px;flex-wrap:wrap}.locActions button{border:0;border-radius:8px;padding:7px 9px;font-weight:800;background:#e9e2d4;color:#173d33}.locActions .danger{background:#833b35;color:#fff}.locEmpty{padding:30px;text-align:center;opacity:.7}
    @media(max-width:760px){#creatorLocationBuilder{padding:0}.locShell{min-height:100dvh;border-radius:0}.locToolbar{grid-template-columns:1fr}.locAreaTop,.locBuildingHead{align-items:flex-start;flex-direction:column}}
  `;
  document.head.appendChild(style);
  modal=document.createElement('section');modal.id='creatorLocationBuilder';modal.hidden=true;
  modal.innerHTML=`<div class="locShell"><header class="locHead"><div><h2>Creator · Locations & Areas</h2><p>Build the estate hierarchy used by asset registers.</p></div><button type="button" data-loc-close>Close</button></header><main class="locBody"><div class="locToolbar"><input data-loc-building placeholder="New building name"><input data-loc-area placeholder="New area name"><select data-loc-type>${CREATOR_AREA_TYPES.map(t=>`<option>${esc(t)}</option>`).join('')}</select><button class="locBtn" type="button" data-loc-add>Add location</button></div><div data-loc-msg></div><div data-loc-list></div></main></div>`;
  document.body.appendChild(modal);
  modal.addEventListener('click',async e=>{
    if(e.target===modal||e.target.closest('[data-loc-close]')){modal.hidden=true;setCreatorDockVisible(true);return}
    if(e.target.closest('[data-loc-add]')){await creatorAddLocation();return}
    const addSub=e.target.closest('[data-loc-add-sub]');if(addSub){const name=prompt('Sub-area name');if(name)await creatorAddSubArea(addSub.dataset.locAddSub,name);return}
    const rename=e.target.closest('[data-loc-rename]');if(rename){const row=locationAreas.find(a=>String(a.id)===rename.dataset.locRename);const name=prompt('Rename area',row?.name||'');if(name&&row)await creatorUpdateArea(row.id,{name:name.trim()});return}
    const archive=e.target.closest('[data-loc-archive]');if(archive){await creatorUpdateArea(archive.dataset.locArchive,{active:false});return}
    const renameSub=e.target.closest('[data-loc-rename-sub]');if(renameSub){const row=locationSubAreas.find(s=>String(s.id)===renameSub.dataset.locRenameSub);const name=prompt('Rename sub-area',row?.name||'');if(name&&row)await creatorUpdateSubArea(row.id,{name:name.trim()});return}
    const archiveSub=e.target.closest('[data-loc-archive-sub]');if(archiveSub){await creatorUpdateSubArea(archiveSub.dataset.locArchiveSub,{active:false});return}
  });
  return modal;
}

function renderCreatorLocationBuilder(){
  const modal=ensureCreatorLocationBuilder(),host=modal.querySelector('[data-loc-list]'),msg=modal.querySelector('[data-loc-msg]');
  msg.innerHTML=locationTablesReady?'':'<p><b>Setup needed:</b> run SUPABASE_LOCATIONS_AREAS_SETUP.sql once in Supabase.</p>';
  const activeAreas=locationAreas.filter(a=>a.active!==false);
  host.innerHTML=buildings.map(b=>{
    const areas=activeAreas.filter(a=>String(a.building_id)===String(b.id));
    return `<section class="locBuilding"><div class="locBuildingHead"><h3>${esc(b.name)}</h3><small>${areas.length} area${areas.length===1?'':'s'}</small></div>${areas.length?areas.map(a=>{
      const subs=locationSubAreas.filter(s=>String(s.area_id)===String(a.id)&&s.active!==false);
      return `<article class="locArea"><div class="locAreaTop"><div><b>${esc(a.name)}</b><br><small>${esc(a.area_type||'Other')}</small></div><div class="locActions"><button data-loc-add-sub="${esc(a.id)}">+ Sub-area</button><button data-loc-rename="${esc(a.id)}">Rename</button><button class="danger" data-loc-archive="${esc(a.id)}">Archive</button></div></div><div class="locSubs">${subs.length?subs.map(s=>`<div>${esc(s.name)} <span class="locActions"><button data-loc-rename-sub="${esc(s.id)}">Rename</button><button class="danger" data-loc-archive-sub="${esc(s.id)}">Archive</button></span></div>`).join(''):'<small>No sub-areas</small>'}</div></article>`
    }).join(''):'<div class="locEmpty">No areas yet.</div>'}</section>`
  }).join('')||'<div class="locEmpty">No buildings found.</div>';
}

async function creatorEnsureBuilding(name){
  let b=buildings.find(x=>x.name.toLowerCase()===name.toLowerCase());if(b)return b;
  const {data,error}=await client.from('buildings').upsert({name,survey_status:'survey_in_progress'},{onConflict:'name'}).select('*').single();if(error)throw error;buildings.push(data);return data;
}
async function creatorAddLocation(){
  if(!locationTablesReady){alert('Run SUPABASE_LOCATIONS_AREAS_SETUP.sql in Supabase first.');return}
  const modal=ensureCreatorLocationBuilder(),buildingName=modal.querySelector('[data-loc-building]').value.trim(),areaName=modal.querySelector('[data-loc-area]').value.trim(),areaType=modal.querySelector('[data-loc-type]').value;
  if(!buildingName||!areaName){alert('Enter both a building and an area name.');return}
  try{const b=await creatorEnsureBuilding(buildingName);const {error}=await client.from('location_areas').upsert({building_id:b.id,name:areaName,area_type:areaType,active:true,updated_by:session?.user?.id||null},{onConflict:'building_id,name'});if(error)throw error;modal.querySelector('[data-loc-area]').value='';await loadLocationDirectory();renderCreatorLocationBuilder();}catch(error){alert(error.message||error)}
}
async function creatorAddSubArea(areaId,name){const {error}=await client.from('location_sub_areas').upsert({area_id:areaId,name:name.trim(),active:true,updated_by:session?.user?.id||null},{onConflict:'area_id,name'});if(error){alert(error.message);return}await loadLocationDirectory();renderCreatorLocationBuilder()}
async function creatorUpdateArea(id,patch){const {error}=await client.from('location_areas').update({...patch,updated_by:session?.user?.id||null,updated_at:new Date().toISOString()}).eq('id',id);if(error){alert(error.message);return}await loadLocationDirectory();renderCreatorLocationBuilder()}
async function creatorUpdateSubArea(id,patch){const {error}=await client.from('location_sub_areas').update({...patch,updated_by:session?.user?.id||null,updated_at:new Date().toISOString()}).eq('id',id);if(error){alert(error.message);return}await loadLocationDirectory();renderCreatorLocationBuilder()}
async function openCreatorLocationBuilder(){if(!LIMEWOOD_CREATOR_MODE)return;setCreatorDockVisible(false);await loadLocationDirectory();const modal=ensureCreatorLocationBuilder();modal.hidden=false;renderCreatorLocationBuilder()}
...
function creatorFieldDefault(index=1){
  return {
    key:`field_${index}`,
    label:`Field ${index}`,
    type:'text',
...
function creatorTemplateEntries(){
  return Object.entries(LOG_TEMPLATES)
    .sort((a,b)=>(a[1].sort_order??100)-(b[1].sort_order??100)||String(a[1].title).localeCompare(String(b[1].title)));
}

function setCreatorDockVisible(visible=true){
  const dock=document.getElementById('lwCreatorDock');
  if(dock)dock.style.display=visible?'flex':'none';
}

function ensureCreatorLogBuilder(){
  let modal=$('creatorLogBuilder');
  if(modal)return modal;

  const style=document.createElement('style');
  style.id='creatorLogBuilderStyle';
  style.textContent=`
    #creatorLogBuilder{position:fixed;inset:0;z-index:2147483646;background:rgba(8,24,20,.78);padding:18px;overflow:auto}
    #creatorLogBuilder[hidden]{display:none!important}
    .clbShell{max-width:1180px;margin:0 auto;background:#f6f2e9;border-radius:22px;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,.35);color:#173d33}
    .clbHead{display:flex;gap:14px;align-items:center;justify-content:space-between;padding:18px 20px;background:#173d33;color:white}
    .clbHead h2{margin:0;font-size:22px}.clbHead p{margin:4px 0 0;opacity:.8;font-size:13px}
    .clbHead button,.clbBtn{border:0;border-radius:12px;padding:10px 14px;font-weight:800;cursor:pointer}
    .clbClose{background:#f6f2e9;color:#173d33}
    .clbBody{display:grid;grid-template-columns:300px 1fr;min-height:620px}
    .clbList{padding:14px;border-right:1px solid rgba(23,61,51,.14);background:#eee8db}
    .clbListTop{display:flex;gap:8px;margin-bottom:10px}.clbListTop button{width:100%;background:#c8a96a;color:#173d33}
    .clbTemplate{display:block;width:100%;text-align:left;border:1px solid rgba(23,61,51,.14);background:white;border-radius:13px;padding:11px;margin:7px 0;color:#173d33}
    .clbTemplate.active{outline:3px solid rgba(200,169,106,.55)}.clbTemplate small{display:block;opacity:.65;margin-top:3px}
    .clbEditor{padding:18px;overflow:auto}
    .clbGrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.clbGrid .wide{grid-column:1/-1}
    .clbEditor label{display:flex;flex-direction:column;gap:5px;font-size:12px;font-weight:800}
    .clbEditor input,.clbEditor textarea,.clbEditor select{box-sizing:border-box;width:100%;padding:10px;border:1px solid rgba(23,61,51,.22);border-radius:10px;background:white;color:#173d33}
    .clbCheck{flex-direction:row!important;align-items:center}.clbCheck input{width:auto}
    .clbFieldsHead{display:flex;align-items:center;justify-content:space-between;margin-top:22px;gap:10px}
    .clbFields{display:flex;flex-direction:column;gap:10px;margin-top:10px}
    .clbField{background:white;border:1px solid rgba(23,61,51,.15);border-radius:14px;padding:12px}
    .clbFieldTop{display:grid;grid-template-columns:1.2fr 1fr .85fr auto;gap:8px;align-items:end}
    .clbFieldMore{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:9px}
    .clbFieldActions{display:flex;gap:5px}.clbFieldActions button{padding:8px 9px;border:0;border-radius:8px;background:#e9e2d4;color:#173d33;font-weight:900}
    .clbFooter{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:18px;padding-top:14px;border-top:1px solid rgba(23,61,51,.15)}
    .clbSave{background:#173d33;color:white}.clbDelete{background:#833b35;color:white}.clbMsg{font-size:12px;font-weight:700}
    @media(max-width:780px){
      #creatorLogBuilder{padding:0}
      .clbShell{border-radius:0;min-height:100dvh}
      .clbBody{grid-template-columns:1fr;min-height:calc(100dvh - 72px)}
      .clbList{border-right:0;border-bottom:1px solid rgba(23,61,51,.14);max-height:220px;overflow:auto}
      .clbGrid,.clbFieldTop,.clbFieldMore{grid-template-columns:1fr}
      .clbEditor{padding:14px 14px 34px}
      .clbFooter{
        position:static;
        background:#f6f2e9;
        margin-top:18px;
      ...
      .clbFooter>div:last-child{display:flex;flex-wrap:wrap;justify-content:flex-end}
      .clbFooter .clbBtn{min-height:48px}
    }
    @media(max-width:780px) and (orientation:landscape){
      .clbBody{grid-template-columns:260px 1fr}
      .clbList{max-height:none;border-right:1px solid rgba(23,61,51,.14);border-bottom:0}
      .clbEditor{padding:14px 14px 30px}
    }
  `;
  document.head.appendChild(style);

  modal=document.createElement('section');
  modal.id='creatorLogBuilder';
  modal.hidden=true;
  modal.innerHTML=`
    <div class="clbShell">
      <header class="clbHead">
        <div><h2>Creator · Log Builder</h2><p>Control every engineering log form without editing app.js.</p></div>
        <button class="clbClose" type="button">Close</button>
      </header>
      <div class="clbBody">
        <aside class="clbList">
          <div class="clbListTop"><button class="clbBtn" data-clb-new type="button">+ New log type</button></div>
          <div data-clb-list></div>
        </aside>
        <main class="clbEditor" data-clb-editor></main>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
  ...
  modal.addEventListener('click',async e=>{
    if(e.target===modal||e.target.closest('.clbClose')){
      modal.hidden=true;
      setCreatorDockVisible(true);
      return;
    }

    const choose=e.target.closest('[data-clb-key]');
    if(choose){
      renderCreatorTemplateEditor(choose.dataset.clbKey);
      return;
    }

    if(e.target.closest('[data-clb-new]')){
      const base='new_log';
      let n=1,key=base;
      while(LOG_TEMPLATES[key])key=`${base}_${++n}`;
      LOG_TEMPLATES[key]={
        key,icon:'📝',title:'New log type',description:'',
        enabled:true,sort_order:creatorTemplateEntries().length*10+10,
        fields:[creatorFieldDefault(1)]
      };
      renderCreatorLogBuilder(key);
      return;
    }

    const add=e.target.closest('[data-clb-add-field]');
    if(add){
      const key=add.dataset.clbAddField;
      const t=LOG_TEMPLATES[key];
      t.fields.push(creatorFieldDefault(t.fields.length+1));
      renderCreatorTemplateEditor(key);
      return;
    }

    const row=e.target.closest('[data-clb-field-index]');
    const action=e.target.closest('[data-clb-field-action]');
    if(row&&action){
      syncCreatorEditorToMemory();
      const key=row.dataset.clbTemplateKey;
      const i=Number(row.dataset.clbFieldIndex);
      const fields=LOG_TEMPLATES[key].fields;
      if(action.dataset.clbFieldAction==='delete')fields.splice(i,1);
      if(action.dataset.clbFieldAction==='up'&&i>0)[fields[i-1],fields[i]]=[fields[i],fields[i-1]];
      if(action.dataset.clbFieldAction==='down'&&i<fields.length-1)[fields[i+1],fields[i]]=[fields[i],fields[i+1]];
      renderCreatorTemplateEditor(key);
      return;
    }

    const save=e.target.closest('[data-clb-save]');
    if(save){
      syncCreatorEditorToMemory();
      await saveCreatorLogTemplate(save.dataset.clbSave);
      return;
    }

    const disable=e.target.closest('[data-clb-toggle]');
    if(disable){
      syncCreatorEditorToMemory();
      const key=disable.dataset.clbToggle;
      LOG_TEMPLATES[key].enabled=!LOG_TEMPLATES[key].enabled;
      await saveCreatorLogTemplate(key);
      return;
    }
  });
...
function creatorFieldCard(templateKey,field,index){
  const typeOptions=['text','number','textarea','select','room','date','time','checkbox']
    .map(t=>`<option value="${t}" ${field.type===t?'selected':''}>${t}</option>`).join('');
  const options=Array.isArray(field.options)?field.options.join(', '):'';
  return `
    <article class="clbField" data-clb-template-key="${esc(templateKey)}" data-clb-field-index="${index}">
      <div class="clbFieldTop">
        <label>Label<input data-clb-f="label" value="${esc(field.label||'')}"></label>
        <label>Key<input data-clb-f="key" value="${esc(field.key||'')}"></label>
        <label>Type<select data-clb-f="type">${typeOptions}</select></label>
        <div class="clbFieldActions">
          <button type="button" data-clb-field-action="up" title="Move up">↑</button>
          <button type="button" data-clb-field-action="down" title="Move down">↓</button>
          <button type="button" data-clb-field-action="delete" title="Delete">×</button>
        </div>
      </div>
      <div class="clbFieldMore">
        <label>Unit / suffix<input data-clb-f="suffix" value="${esc(field.suffix||'')}" placeholder="mg/L, °C, etc."></label>
        <label>Placeholder<input data-clb-f="placeholder" value="${esc(field.placeholder||'')}"></label>
        <label>Dropdown options<input data-clb-f="options" value="${esc(options)}" placeholder="Normal, Review, Action"></label>
      </div>
      <label class="clbCheck" style="margin-top:9px"><input type="checkbox" data-clb-f="required" ${field.required?'checked':''}> Required field</label>
    </article>`;
}

function renderCreatorLogBuilder(selectedKey=''){
  const modal=ensureCreatorLogBuilder();
  const entries=creatorTemplateEntries();
  const list=modal.querySelector('[data-clb-list]');

  list.innerHTML=entries.length?entries.map(([key,t])=>`
    <button type="button" class="clbTemplate ${key===selectedKey?'active':''}" data-clb-key="${esc(key)}">
      <b>${esc(t.icon||'📝')} ${esc(t.title||key)}</b>
      <small>${t.enabled===false?'Disabled':'Live'} · ${t.fields?.length||0} fields</small>
    </button>`).join(''):'<p>No log templates found.</p>';

  const key=selectedKey&&LOG_TEMPLATES[selectedKey]?selectedKey:entries[0]?.[0]||'';
  renderCreatorTemplateEditor(key);
}

function renderCreatorTemplateEditor(key){
  const modal=ensureCreatorLogBuilder();
  const editor=modal.querySelector('[data-clb-editor]');
  const t=LOG_TEMPLATES[key];

  modal.querySelectorAll('[data-clb-key]').forEach(b=>b.classList.toggle('active',b.dataset.clbKey===key));

  if(!t){
    editor.innerHTML='<p>Select a log type or create a new one.</p>';
    return;
  }

  editor.innerHTML=`
    <div class="clbGrid" data-clb-template="${esc(key)}">
      <label>Icon<input data-clb-t="icon" value="${esc(t.icon||'📝')}" maxlength="8"></label>
      <label>Internal key<input data-clb-t="key" value="${esc(key)}" disabled></label>
      <label class="wide">Log title<input data-clb-t="title" value="${esc(t.title||'')}"></label>
      <label class="wide">Description<textarea data-clb-t="description" rows="2">${esc(t.description||'')}</textarea></label>
      <label>Sort order<input data-clb-t="sort_order" type="number" value="${esc(t.sort_order??100)}"></label>
      <label class="clbCheck"><input data-clb-t="enabled" type="checkbox" ${t.enabled!==false?'checked':''}> Available to engineers</label>
    </div>

    <div class="clbFieldsHead">
      <div><h3 style="margin:0">Fields</h3><small>Drag-free because browsers enjoy suffering. Use ↑ and ↓.</small></div>
      <button class="clbBtn" style="background:#c8a96a;color:#173d33" type="button" data-clb-add-field="${esc(key)}">+ Add field</button>
    </div>

    <div class="clbFields">
      ${(t.fields||[]).map((field,index)=>creatorFieldCard(key,field,index)).join('')||'<p>No fields yet.</p>'}
    </div>

    <div class="clbFooter">
      <span class="clbMsg" data-clb-msg>${logTemplateCloudReady?'Connected to Supabase':'Using built-in templates until log_templates is created'}</span>
      <div style="display:flex;gap:8px">
        <button class="clbBtn clbDelete" type="button" data-clb-toggle="${esc(key)}">${t.enabled===false?'Enable':'Disable'} log</button>
        <button class="clbBtn clbSave" type="button" data-clb-save="${esc(key)}">Save changes</button>
      </div>
    </div>`;
}

function syncCreatorEditorToMemory(){
  const modal=$('creatorLogBuilder');
  const root=modal?.querySelector('[data-clb-template]');
  if(!root)return;

  const key=root.dataset.clbTemplate;
  const t=LOG_TEMPLATES[key];
  if(!t)return;

  root.querySelectorAll('[data-clb-t]').forEach(el=>{
    const prop=el.dataset.clbT;
    if(prop==='key')return;
    t[prop]=el.type==='checkbox'?el.checked:(prop==='sort_order'?Number(el.value||100):el.value);
  });

  const fields=[];
  modal.querySelectorAll(`[data-clb-template-key="${CSS.escape(key)}"]`).forEach(card=>{
    const field={};
    card.querySelectorAll('[data-clb-f]').forEach(el=>{
      const prop=el.dataset.clbF;
      if(prop==='required')field.required=el.checked;
      else if(prop==='options')field.options=el.value.split(',').map(x=>x.trim()).filter(Boolean);
      else field[prop]=el.value;
    });
    field.key=creatorSlug(field.key||field.label)||`field_${fields.length+1}`;
    fields.push(field);
  });
...
async function saveCreatorLogTemplate(key){
  const t=LOG_TEMPLATES[key];
  const msg=$('creatorLogBuilder')?.querySelector('[data-clb-msg]');
  if(!t)return;

  if(!logTemplateCloudReady){
    if(msg)msg.textContent='Run the supplied Supabase SQL once, then reload Creator.';
    alert('The log_templates table is not available yet. Run the Creator Log Builder SQL in Supabase once, then reload the app.');
    return;
  }
  ...
  const payload={
    ...
    sort_order:Number(t.sort_order||100),
  ...
  if(error){
    ...
    alert(`Could not save log template: ${error.message}`);
    return;
  }

  if(msg)msg.textContent='Saved. Engineers will see the new form on refresh.';
  renderCreatorLogBuilder(key);
  renderLogTypes();
}

async function openCreatorLogBuilder(){
  if(!LIMEWOOD_CREATOR_MODE)return;
  setCreatorDockVisible(false);
  await loadLogTemplates();
  const modal=ensureCreatorLogBuilder();
  modal.hidden=false;
  renderCreatorLogBuilder();
}
...
function renderLogTypes(){
 ...
 host.innerHTML=Object.entries(LOG_TEMPLATES).filter(([,t])=>t.enabled!==false).sort((a,b)=>(a[1].sort_order??100)-(b[1].sort_order??100)).map(([key,t])=>`<button type="button" data-log-type="${esc(key)}"><span>${t.icon}</span><b>${esc(t.title)}</b><small>${esc(t.description)}</small></button>`).join('');
...
function logFieldHtml(field){
 ...
 else if(field.type==='checkbox')control=`<input id="${id}" data-log-key="${esc(field.key)}" type="checkbox" ${req}>`;
...
function beginLog(type){
 const t=LOG_TEMPLATES[type];if(!t||t.enabled===false)return;currentLogType=type;showView('logs');$('logTypePanel').hidden=true;$('logHistoryPanel').hidden=true;$('logFormPanel').hidden=false;$('logFormTitle').textContent=t.title;$('logFormDescription').textContent=t.description;$('dynamicLogFields').innerHTML=t.fields.map(logFieldHtml).join('');$('logAutoTime').textContent=new Date().toLocaleString();$('logAutoUser').textContent=logUserLabel();$('logSaveMessage').hidden=true;window.scrollTo({top:0,behavior:'smooth'});
}

function collectLogPayload(){const payload={};document.querySelectorAll('#dynamicLogFields [data-log-key]').forEach(el=>{payload[el.dataset.logKey]=el.type==='checkbox'?el.checked:el.value});return payload}
...
function valveNumberLabel(v){
 const raw=String(v?.tag||'').trim();
 const match=raw.match(/(?:^|[-\s])V(?:ALVE)?[-\s]*0*(\d+)(?:$|[-\s])/i)||raw.match(/^0*(\d+)$/);
 return match?`Valve ${Number(match[1])}`:(raw||'Untagged valve')
}

function valveSortNumber(v){
 const m=valveNumberLabel(v).match(/(\d+)/);
 return m?Number(m[1]):999999
}

function filteredValveRows(){
 const q=$('valveSearch').value.toLowerCase(),room=$('valveRoom').value,pos=$('valvePosition').value;
 return valveRecords.filter(v=>{
  const hay=[v.tag,v.plant_room,v.service_duty,v.valve_type,v.size,v.location,v.isolation_purpose,v.notes,v.asset_code].join(' ').toLowerCase();
  return(!q||hay.includes(q))&&(!room||samePlantRoom(v.plant_room,room))&&(!pos||v.normal_position===pos)
 }).sort((a,b)=>valveSortNumber(a)-valveSortNumber(b)||String(a.tag).localeCompare(String(b.tag)))
}

function valvePositionClass(position){
 const p=String(position||'').toLowerCase();
 return p.includes('closed')?'closed':p.includes('open')?'open':'other'
}

function renderValves(){
 const body=$('valveRows');if(!body)return;
 const room=$('valveRoom').value,rooms=allPlantRoomNames();

 renderValveRoomButtons(rooms);

 $('valveTotal').textContent=valveRecords.length;
 $('valveOpen').textContent=valveRecords.filter(v=>String(v.normal_position).toLowerCase().includes('open')).length;
 $('valveClosed').textContent=valveRecords.filter(v=>String(v.normal_position).toLowerCase().includes('closed')).length;
 $('valveVerifyDue').textContent=valveRecords.filter(v=>!v.last_verified||(Date.now()-new Date(v.last_verified+'T12:00:00'))>365*86400000).length;

 $('valveDirectoryPanel').hidden=Boolean(room);
 $('valveRegisterPanel').hidden=!room;
 $('valvePageTitle').textContent=room?room.replace(/ Plant Room$/i,'')+' Valve Register':'Valve Register';
 $('valvePageDescription').textContent=room?'Select a valve to view what it is, what it does and its normal position.':'Choose a plant room to open its dedicated valve register.';
 ...
 const rows=filteredValveRows();

 $('valveRoomTitle').textContent=room;
 $('valveRoomCount').textContent=`${rows.length} valve${rows.length===1?'':'s'} shown · ${valveRecords.filter(v=>samePlantRoom(v.plant_room,room)).length} registered`;
...
function valveDetailRows(v){
 return [
  ['Physical label',valveNumberLabel(v)],
  ['Plant room',v.plant_room],
  ['Size',v.size],
  ['Valve type',v.valve_type],
  ['Service / system',v.service_duty],
  ['What it does',v.isolation_purpose],
  ['Normal position',v.normal_position],
  ['Exact location',v.location],
  ['Linked asset',v.asset_code],
  ['Last verified',v.last_verified],
  ['Notes',v.notes]
 ].filter(([,value])=>value)
}

function valveRecordUrl(v){
 const u=new URL(location.href);
 u.search='';
 u.hash='';
 u.searchParams.set('valve',v.tag||v.id);
 if(v.plant_room)u.searchParams.set('room',v.plant_room);
 return u.toString()
}

function openValveDetail(id){
 const v=valveRecords.find(x=>String(x.id)===String(id));if(!v)return;
 currentValveDetailId=String(v.id);
 $('valveDetailTitle').textContent=valveNumberLabel(v);
 $('valveDetailRoom').textContent=v.plant_room||'Plant room to confirm';
 $('valveDetailBody').innerHTML=valveDetailRows(v).map(([label,value])=>`<article><span>${esc(label)}</span><b>${esc(value)}</b></article>`).join('');
 $('valveDetailQr').hidden=true;
 $('valveQrCode').innerHTML='';
 const list=filteredValveRows(),idx=list.findIndex(x=>String(x.id)===String(v.id));
 $('previousValve').disabled=idx<=0;
 $('nextValve').disabled=idx<0||idx>=list.length-1;
 openOpsModal('valveDetailModal')
}

function stepValveDetail(direction){
 const list=filteredValveRows(),idx=list.findIndex(x=>String(x.id)===String(currentValveDetailId)),next=list[idx+direction];
 if(next)openValveDetail(next.id)
}

function showValveQr(){
 const v=valveRecords.find(x=>String(x.id)===String(currentValveDetailId));if(!v)return;
 const box=$('valveQrCode');
 box.innerHTML='';
 $('valveDetailQr').hidden=false;
 if(typeof QRCode==='undefined')box.textContent='QR library could not load.';
 else new QRCode(box,{text:valveRecordUrl(v),width:220,height:220,correctLevel:QRCode.CorrectLevel.M})
}

function openValveFromUrl(){
 const params=new URLSearchParams(location.search),key=params.get('valve');
 if(!key)return;
 const room=params.get('room')||'',v=valveRecords.find(x=>String(x.tag)===key||String(x.id)===key);
 if(v){
  showView('valves');
  openValveRoomRegister(room||v.plant_room);
  openValveDetail(v.id)
 }
}

function openValve(id=''){
 editingValveId=id;
 const v=valveRecords.find(x=>String(x.id)===String(id))||{};
 $('valveModalTitle').textContent=id?'Edit valve':'Add valve';
 $('vTag').value=v.tag||'';
 $('vRoom').value=v.plant_room||$('valveRoom').value||$('vRoom').options[0]?.value||'';
 $('vAsset').value=v.asset_code||'';
 $('vService').value=v.service_duty||'';
 $('vType').value=v.valve_type||'';
 $('vSize').value=v.size||'';
 $('vPosition').value=v.normal_position||'Open';
 $('vVerified').value=v.last_verified||'';
 $('vLocation').value=v.location||'';
 $('vIsolation').value=v.isolation_purpose||'';
 $('vNotes').value=v.notes||'';
 $('deleteValve').hidden=!id;
 openOpsModal('valveModal')
}
...
async function saveValveRecord(){
 ...
 const rec={
  tag:$('vTag').value.trim(),
  plant_room:$('vRoom').value,
  asset_code:$('vAsset').value||null,
  service_duty:$('vService').value.trim(),
  valve_type:$('vType').value.trim(),
  size:$('vSize').value.trim(),
  normal_position:$('vPosition').value,
  last_verified:$('vVerified').value||null,
  location:$('vLocation').value.trim(),
  isolation_purpose:$('vIsolation').value.trim(),
 ...
 else valveRecords.push(data);

 closeOpsModal('valveModal');
 renderValves();
}
...
function exportPpmCsv(){
 csvDownload(
  'Limewood-PPM-Register.csv',
  ['Asset ID','Asset','Plant room','Frequency','Last completed','Next due','Status','Assigned to','Task','Notes'],
  ppmRecords.map(r=>{
   const a=assetForCode(r.asset_code);
   return[r.asset_code,a?.name,a?.room,r.frequency,r.last_completed,r.next_due,ppmStatus(r),r.assigned_to,r.task,r.notes]
  })
 )
}

function exportValveCsv(){
 csvDownload(
  'Limewood-Valve-Register.csv',
  ['Tag','Plant room','Linked asset','Service duty','Type','Size','Normal position','Location','Isolation purpose','Last verified','Notes'],
  valveRecords.map(v=>[v.tag,v.plant_room,v.asset_code,v.service_duty,v.valve_type,v.size,v.normal_position,v.location,v.isolation_purpose,v.last_verified,v.notes])
 )
}
...
function toView(row) {
  ...
  const area = locationAreas.find(r => r.id === row.area_id);
  const subArea = locationSubAreas.find(r => r.id === row.sub_area_id);
  const building = buildings.find(b => b.id === (row.building_id || area?.building_id || pr?.building_id));
  const displayRoom = area?.name || pr?.name || 'To be confirmed';
  return {
    uuid: row.id,
    id: row.asset_code,
    name: row.asset_name,
    ...
    areaName: area?.name || pr?.name || '',
    areaId: area?.id || null,
    areaType: area?.area_type || (pr ? 'Plant Room' : ''),
...
async function loadDocumentCentre() {
  ...
  const ordinary = await Promise.all((docRes.data || []).map(async d => {
    ...
    return {
      ...
      number:looksNumbered ? parts.shift() : '',
      ...
      buildingId:'',
      plantRoomId:'',
      assetIds:d.asset_id ? [d.asset_id] : [],
  ...
  const sops = sopTablesReady ? await Promise.all((sopRes.data || []).map(async row => {
    ...
    return {
      ...
      plantRoomId:row.plant_room_id || '',
      assetIds:links.filter(x=>x.sop_id===row.id).map(x=>x.asset_id),
  ...
  if (!sopTablesReady) {
    console.warn('Controlled SOP tables are unavailable. SOP uploads will use the standard document library fallback.');
  }
}
...
function populateDocumentFilters() {
  ...
  docEls.buildingFilter.innerHTML =
    '<option value="">All buildings</option>' +
    buildings.map(b=>`<option value="${esc(b.id)}">${esc(b.name)}</option>`).join('');

  if (buildings.some(b=>b.id===current)) {
    docEls.buildingFilter.value=current;
  }

  docEls.building.innerHTML =
    '<option value="">No building selected</option>' +
    buildings.map(b=>`<option value="${esc(b.id)}">${esc(b.name)}</option>`).join('');

  docEls.asset.innerHTML =
    '<option value="">No asset selected</option>' +
    assets.map(a=>`<option value="${esc(a.uuid)}">${esc(a.id)} · ${esc(a.name)}</option>`).join('');
...
function updateDocumentPlantRooms() {
  ...
  docEls.plantRoom.innerHTML =
    '<option value="">No plant room selected</option>' +
    rooms.map(r=>`<option value="${esc(r.id)}">${esc(r.name)}</option>`).join('');
}

function filteredDocuments() {
  const q=docEls.search.value.trim().toLowerCase();
  const type=docEls.typeFilter.value;
  const building=docEls.buildingFilter.value;
  const status=docEls.statusFilter.value;
  ...
  return libraryDocuments.filter(d=>{
    ...
    return (!q||hay.includes(q))
      && (!type||d.type===type)
      && (!building||d.buildingId===building)
...
function renderDocuments() {
  ...
  docEls.grid.innerHTML=rows.map(d=>{
    ...
    const open=d.url
      ? `<a class="documentOpen" href="${esc(d.url)}" target="_blank" rel="noopener">Open document</a>`
      : '<span class="documentUnavailable">File unavailable</span>';

    return `<article class="documentCard">
      <div class="documentCardTop">
        <span class="documentType">${esc(documentTypeLabel(d.type))}</span>
        <span class="documentStatus ${esc(d.status)}">${esc((d.status||'approved').replace('_',' '))}</span>
      </div>
      <h4>${esc(d.title)}</h4>
      <p class="documentNumber">${esc(d.number || 'No document number')}</p>
      <dl>
        <div><dt>Category</dt><dd>${esc(d.category||'General')}</dd></div>
        <div><dt>Revision</dt><dd>${esc(d.revision||'—')}</dd></div>
        <div><dt>Building</dt><dd>${esc(building||'Estate-wide')}</dd></div>
        <div><dt>Related asset</dt><dd>${esc(assetNames||'None')}</dd></div>
      </dl>
      ${open}
    </article>`;
...
function showDocuments(type='') {
  ...
  const labels={
    sop:'SOP Library',
    rams:'RAMS Library',
    manual:'Manufacturer Manuals',
    schematic:'Hydraulic Schematics',
    report:'Reports'
  };
...
function openDocumentUpload() {
  ...
  docEls.number.value='';
  docEls.docTitle.value='';
  docEls.category.value='';
  docEls.revision.value='1';
  docEls.status.value='draft';
  docEls.author.value='';
  docEls.approvedBy.value='';
  docEls.issueDate.value='';
  docEls.reviewDate.value='';
  docEls.building.value='';

  updateDocumentPlantRooms();

  docEls.asset.value='';
  docEls.description.value='';
  docEls.changeSummary.value='';
  docEls.file.value='';
  docEls.message.textContent='';

  docEls.modal.classList.add('open');
  docEls.modal.setAttribute('aria-hidden','false');
  document.body.classList.add('modal-open');
}

function closeDocumentUpload() {
  docEls.modal.classList.remove('open');
...
function friendlyUploadError(error) {
  ...
  if (/row-level security|violates row-level security/i.test(message)) {
    return 'Supabase blocked the upload. Check the authenticated-user policies for the asset-files bucket and document tables.';
  }

  if (/bucket.*not found|not found.*bucket/i.test(message)) {
    return 'The Supabase storage bucket is missing. It must be named asset-files.';
  }

  if (/payload too large|maximum allowed size|file size/i.test(message)) {
    return 'That file is too large for the current Supabase upload limit.';
  }

  if (/duplicate key|unique constraint/i.test(message)) {
    return 'That document number and revision already exist.';
  }
...
async function removeUploadedFile(path) {
  ...
  try {
    ...
    console.warn('Could not clean up failed upload', error);
  }
}
...
async function saveControlledSop({file,title,number,path}) {
  ...
  const payload={
    sop_number:number,
    title,
    category:docEls.category.value.trim()||'General',
    description:docEls.description.value.trim()||null,
    revision,
    status:docEls.status.value,
    author:docEls.author.value.trim()||null,
    approved_by:docEls.approvedBy.value.trim()||null,
    issue_date:docEls.issueDate.value||null,
    review_date:docEls.reviewDate.value||null,
    building_id:docEls.building.value||null,
    plant_room_id:docEls.plantRoom.value||null,
    current_file_path:path,
    current_file_name:file.name,
    current_file_type:file.type||null,
    updated_by:session.user.id
  };
  ...
  const {data:existingRevision,error:revisionLookupError}=await client
    .from('sop_revisions')
    .select('id')
    .eq('sop_id',sop.id)
  ...
  if(existingRevision) {
    throw new Error(`Revision ${revision} already exists for ${number}. Enter a new revision number.`);
  }
  ...
  const {error:revError}=await client.from('sop_revisions').insert({
    ...
    change_summary:docEls.changeSummary.value.trim()||null,
    uploaded_by:session.user.id
  });
  ...
  if(docEls.asset.value) {
    const {error:linkError}=await client.from('asset_sops').upsert({
      asset_id:docEls.asset.value,
      sop_id:sop.id
    },{
      onConflict:'asset_id,sop_id'
    });
...
async function saveLibraryDocument() {
  ...
  try {
    ...
    docEls.save.disabled=true;
    docEls.save.textContent='Uploading…';

    docEls.message.classList.remove('success');
    docEls.message.textContent=`Uploading ${file.name}…`;

    const folder=type==='sop'
      ? `sops/${cleanName(number)}`
      : `documents/${cleanName(type||'other')}/${crypto.randomUUID()}`;
    ...
    closeDocumentUpload();
    showDocuments(documentTypeContext || type);
    ...
    docEls.save.disabled=false;
    docEls.save.textContent='Upload to Document Centre';
  }
}
...
async function ensurePlantRoom(roomName) {
  ...
  if (!plantRooms.some(r => r.id === room.id)) {
    plantRooms.push(room);
  }
...
async function seedExistingAssets() {
  ...
  const existingCodes = new Set(
    (existing || []).map(a => a.asset_code)
  );

  const missingAssets = staticBase.filter(
    a => !existingCodes.has(a.id)
  );
  ...
  const uniqueRooms = [
    ...new Set(
      missingAssets.map(a => a.room).filter(Boolean)
    )
  ];

  for (const roomName of uniqueRooms) {
    await ensurePlantRoom(roomName);
  }
...
function isMissing(a) {
  const unknown = v =>
    !v ||
    /to be confirmed|unknown|not assessed|manual to be added/i.test(v);
...
function populateFilters() {
  const currentRoom = els.room.value;
  const currentCategory = els.category.value;

  const rooms = [
    ...new Set(
      assets
        .map(a=>canonicalPlantRoomName(a.room))
        .filter(Boolean)
    )
  ].sort();
  ...
  els.room.innerHTML =
    '<option value="">All plant rooms</option>' +
    rooms.map(x => `<option>${esc(x)}</option>`).join('');
...
function updateStats() {
  ...
  els.surveyedCount.textContent =
    assets.filter(a =>
      ['Surveyed','Verified','Operational'].includes(a.status)
    ).length;

  els.reviewCount.textContent =
    assets.filter(a =>
      ['Needs review','Limited access'].includes(a.status)
    ).length;

  els.photoCount.textContent =
    assets.reduce((n,a) => n + a.photos.length, 0);

  const setCount = (id, room) => {
    const el=$(id);

    if(el) {
      el.textContent=
        `${assets.filter(a=>samePlantRoom(a.room,room)).length} assets`;
    }
  };

  setCount('mainCount','Main House Plant Room');
  setCount('staffCount','Staff House Plant Room');
  setCount('coachCount','Coach House Plant Room');
  setCount('forestCount','Forest Lodges Plant Room');

  const missing=assets.filter(isMissing).length;

  const critical=assets.filter(a=>
    ['high','critical'].includes(
      String(a.criticality||'').toLowerCase()
    )
  ).length;
  ...
  if(els.dashboardDocumentCount) {
    els.dashboardDocumentCount.textContent=libraryDocuments.length;
  }

  const complete=assets.length
    ? Math.round(((assets.length-missing)/assets.length)*100)
    : 0;

  if(els.qualityPercent){
    els.qualityPercent.textContent=`${complete}%`;

    const ring=els.qualityPercent.closest('.qualityRing');

    if(ring) {
      ring.style.setProperty('--quality',`${complete}%`);
    }
  }
...
function renderRecentActivity() {
  ...
  const assetEvents=assets.map(a=>({
    type:'asset',
    title:`${a.id} · ${a.name}`,
    detail:`${a.room} · ${a.status}`,
    date:a.raw?.updated_at||a.raw?.created_at||'',
    icon:'⚙️'
  }));

  const documentEvents=libraryDocuments.map(d=>({
    type:'document',
    title:d.title||'Document uploaded',
    detail:`${documentTypeLabel(d.type)}${d.number?' · '+d.number:''}`,
    date:d.createdAt||'',
    icon:'📄'
  }));

  const rows=[...assetEvents,...documentEvents]
    .sort((a,b)=>String(b.date).localeCompare(String(a.date)))
    .slice(0,6);

  els.recentActivity.innerHTML=rows.length
    ? rows.map(x=>`
      <div class="activityItem">
        <span class="activityIcon">${x.icon}</span>
        <div>
          <b>${esc(x.title)}</b>
    ...
    `).join('')
    : '<p class="emptyState">No recent activity recorded yet.</p>';
}

function filtered() {
  const q = els.search.value.trim().toLowerCase();
  const effectiveRoom = registerForcedRoom || els.room.value;
  const disciplineTerms =
    registerDisciplineTerms.map(x=>String(x).toLowerCase());
  ...
  return assets.filter(a => {
    ...
    const roomMatches =
      !effectiveRoom ||
      samePlantRoom(a.room,effectiveRoom);

    const disciplineMatches =
      !disciplineTerms.length ||
      disciplineTerms.some(term=>hay.includes(term));

    return (!q || hay.includes(q))
      && roomMatches
      && disciplineMatches
      && (!els.category.value || a.category === els.category.value)
      && (
...
function cardImage(a) {
  return a.photos[0]?.url || localPhoto(a);
}

function render() {
  visibleRows = filtered().sort(
    (a,b)=>
  ...
  if(registerDisciplineTitle) {
    registerTitle.textContent = registerDisciplineTitle;
  }

  els.grid.innerHTML = visibleRows.map(a => `
    <article
      class="card assetCompactCard assetTextTile"
      tabindex="0"
      data-id="${esc(a.id)}"
    >
      <div class="cardBody">
        <div class="topline">
          <span class="badge">${esc(a.id)}</span>
          <span class="status">${esc(a.status)}</span>
        </div>

        <h4>${esc(a.name)}</h4>

        <div class="assetTileRoom">
          ${esc(a.room || 'Plant room not set')}
        </div>

        <div class="assetTileMeta">
          <span>
            <small>System</small>
            <b>${esc(a.system || 'To confirm')}</b>
          </span>

          <span>
            <small>Category</small>
            <b>${esc(a.category || 'Uncategorised')}</b>
          </span>
        </div>

        <div class="assetTileFooter">
          <span class="criticalityPill">
            ${esc(a.criticality || 'Medium')} criticality
          </span>

          <span class="assetTileOpen">
            Open →
          </span>
        </div>
      </div>
      ...
      <div class="assetHoverPreview" aria-hidden="true">
        <div class="assetPreviewRow">
          ...
          <b>${esc(a.manufacturer || 'To confirm')}</b>
        </div>

        <div class="assetPreviewRow">
          <span>Model</span>
          <b>${esc(a.model || 'To confirm')}</b>
        </div>

        <div class="assetPreviewRow">
          <span>Condition</span>
          <b>${esc(a.condition || 'To confirm')}</b>
        </div>

        <div class="assetPreviewRow">
          <span>PPM</span>
          <b>${esc(a.ppm || 'To confirm')}</b>
        </div>

        <button
          class="openAssetBtn compactOpenAssetBtn"
          data-id="${esc(a.id)}"
        >
          Open asset →
        </button>
  ...
  `).join('') ||
  '<div class="emptyState">No assets match those filters.</div>';
}
...
function detailRows(a) {
  return [
    ['Plant room',a.room],
    ['Category',a.category],
    ['System / duty',a.system],
    ['Manufacturer',a.manufacturer],
    ['Model',a.model],
    ['Serial number',a.serial],
    ['Condition',a.condition],
    ['Criticality',a.criticality],
    ['Electrical isolation',a.electricalIsolation],
    ['Mechanical isolation',a.mechanicalIsolation],
    ['Isolation procedure',a.isolationProcedure],
    ['PPM frequency',a.ppm]
  ];
}
...
function renderGallery() {
  ...
  const items = [
    ...cloud.map(p => ({
      src:p.url,
      caption:p.caption || p.photo_type || 'Asset photograph'
    })),
    ...(
      local.includes('placeholder')
        ...
        : [{
            src:local,
            caption:'Original survey photograph'
          }]
    )
  ];

  els.gallery.innerHTML = items.length
    ? items.map(x => `
      <figure>
        <img src="${esc(x.src)}" alt="${esc(x.caption)}">
    ...
    : `
      <figure>
        <img
          src="assets/images/asset-placeholder.svg"
          alt="No photograph"
        >
        <figcaption>No photograph uploaded</figcaption>
      </figure>
    `;
}
...
function renderLinks() {
  ...
  if (current.manufacturerUrl) {
    links.push(
      `<a href="${esc(current.manufacturerUrl)}" target="_blank" rel="noopener">Manufacturer website</a>`
    );
  }

  if (current.manualUrl) {
    links.push(
      `<a href="${esc(current.manualUrl)}" target="_blank" rel="noopener">Online manual</a>`
    );
  }
  ...
  for (const d of current.documents || []) {
    if (d.url) {
      links.push(
        `<a href="${esc(d.url)}" target="_blank" rel="noopener">${esc(d.title)} · ${esc(d.document_type)}</a>`
      );
    }
  }
  ...
  els.links.innerHTML =
    links.join('') ||
    '<small>No linked documents yet.</small>';
}
...
function renderDetails() {
  ...
  els.modalDetails.innerHTML =
    detailRows(current)
      .map(([k,v]) =>
        `<div><small>${esc(k)}</small>${esc(v || 'To be confirmed')}</div>`
      )
      .join('');
  ...
  if(els.assetQrBtn) {
    els.assetQrBtn.hidden=!current.id;
  }

  if(els.assetBmsBtn){
    const key=bmsKeyForRoom(current.room);

    els.assetBmsBtn.hidden=
      ![
        'staff',
        'main',
        'coach',
    ...
    els.assetBmsBtn.dataset.bmsKey=key;
  }
}

let maintenanceHistoryRecords = [];
...
function renderMaintenanceHistory() {
  ...
  const type =
    String($('historyTypeFilter')?.value || '');

  const days =
    String($('historyDateFilter')?.value || 'all');

  let records = [...maintenanceHistoryRecords];

  if (search) {
    records = records.filter(
      r=>historyRecordSearchText(r).includes(search)
    );
  }

  if (type) {
    records = records.filter(
      r=>String(r.work_type || 'Maintenance') === type
    );
  }
  ...
  if (days !== 'all') {
    ...
    records = records.filter(r => {
      const t = r.work_date
        ? new Date(
            `${String(r.work_date).slice(0,10)}T12:00:00`
          ).getTime()
        : 0;
  ...
  const resultMeta = $('historyResultMeta');

  if (resultMeta) {
    resultMeta.textContent =
      `${records.length} of ${maintenanceHistoryRecords.length} record${
        maintenanceHistoryRecords.length === 1 ? '' : 's'
      }`;
  }
  ...
  if (!records.length) {
    list.innerHTML = `
      <div class="historyEmpty">
        <b>No matching maintenance records</b>
  ...
  list.innerHTML = records.map((record, index) => {
    const followUp = !!record.follow_up_required;
    const workType = record.work_type || 'Maintenance';
    const description = record.description || 'Maintenance record';
    const engineer = record.engineer_name || 'Not recorded';
    const findings = record.findings || 'No findings recorded.';
    const actions = record.actions_taken || 'No actions recorded.';
    const open = index === 0 ? ' open' : '';

    return `
      <details
        class="historyRecord${followUp ? ' needsFollowUp' : ''}"
        ${open}
      >
        <summary>
          <span
            class="historyTimelineDot"
            aria-hidden="true"
          ></span>

          <div class="historyDateBlock">
            <b>${esc(historyDateLabel(record.work_date))}</b>
            <small>${esc(workType)}</small>
          </div>

          <div class="historyRecordTitle">
            <strong>${esc(description)}</strong>
            <span>${esc(engineer)}</span>
          </div>
        ...
        <div class="historyRecordBody">
          <div class="historyInfoGrid">
            <article>
              <span>Engineer</span>
              <b>${esc(engineer)}</b>
            </article>
          ...
          ${followUp ? `
            <div class="historyFollowUpBox">
              <div><b>Further action required</b><span>This maintenance record has been flagged for follow-up.</span></div>
              <strong>${record.follow_up_date ? esc(historyDateLabel(record.follow_up_date)) : 'Date not set'}</strong>
            </div>` : ''}
        </div>
      </details>`;
  }).join('');
...
function populateMaintenanceHistory(records) {
  maintenanceHistoryRecords = records || [];

  $('historyAssetId').textContent = current.id || 'ASSET';
  $('historyAssetName').textContent = current.name || 'Asset maintenance history';

  const meta = [current.room, current.category, current.systemDuty || current.system || current.duty, current.manufacturer]
    .filter(Boolean).join(' · ');
  $('historyAssetMeta').textContent = meta || 'Engineering asset record';

  $('historyTotal').textContent = maintenanceHistoryRecords.length;
  $('historyFollowUp').textContent = maintenanceHistoryRecords.filter(r => r.follow_up_required).length;
  $('historyCompleted').textContent = maintenanceHistoryRecords.filter(r => !r.follow_up_required).length;
  $('historyLastDate').textContent = maintenanceHistoryRecords.length ? historyShortDate(maintenanceHistoryRecords[0].work_date) : '—';

  const typeFilter = $('historyTypeFilter');
  if (typeFilter) {
    const types = [...new Set(maintenanceHistoryRecords.map(r => r.work_type || 'Maintenance'))].sort();
    typeFilter.innerHTML = '<option value="">All maintenance types</option>' +
  ...
  const bmsBtn = $('historyBmsBtn');
  if (bmsBtn) {
    const key = bmsKeyForRoom(current.room);
    const available = ['staff','main','coach','spa','green','crescent','pavilion'].includes(key);
    bmsBtn.hidden = !available;
    bmsBtn.dataset.bmsKey = key;
  }
...
async function openMaintenanceHistory() {
  ...
  try {
    ...
    const records = data || [];
    populateMaintenanceHistory(records);
...
function setEditing(on) {
  ...
  if (on) {
    els.editFields.innerHTML = fields.map(([key,label]) => {
      if(key==='buildingName') return `<label>${esc(label)}<select data-field="buildingName" id="assetBuildingSelect"></select></label>`;
      if(key==='areaName') return `<label>${esc(label)}<select data-field="areaName" id="assetAreaSelect"></select></label>`;
      if(key==='subAreaName') return `<label>${esc(label)}<select data-field="subAreaName" id="assetSubAreaSelect"></select></label>`;
      return `<label>${esc(label)}<input data-field="${key}" value="${esc(current[key] || '')}"></label>`;
    }).join('');
    populateAssetLocationEditor();
...
function nextAssetCode() {
  const numbers = assets
    .map(a => String(a?.id || '').trim().match(/^FLP-(\d+)$/i))
    .filter(Boolean)
    .map(m => Number(m[1]))
    .filter(Number.isFinite);
  const next = (numbers.length ? Math.max(...numbers) : 0) + 1;
  return `FLP-${String(next).padStart(3,'0')}`;
}

function newAsset() {
  current = {uuid:null,id:nextAssetCode(),name:'',room:els.room.value || '',buildingName:'',areaName:els.room.value || '',areaId:null,areaType:'',subAreaName:'',subAreaId:null,exactLocation:'',category:'',system:'',manufacturer:'',model:'',serial:'',status:'Needs review',condition:'Unknown',criticality:'Medium',electricalIsolation:'',mechanicalIsolation:'',isolationProcedure:'',ppm:'',notes:'',manufacturerUrl:'',manualUrl:'',photos:[],documents:[]};
...
function assetReturnTarget(){
  const params=new URLSearchParams(location.search);
  const explicit=params.get('return')||'';
  if(explicit && explicit.startsWith('/') && !explicit.startsWith('//')) return explicit;
  try{
    const ref=new URL(document.referrer);
    if(ref.origin===location.origin && ref.pathname && ref.pathname!=='/' && ref.pathname!==location.pathname){
      return ref.pathname+ref.search;
    }
  }catch(_){}
  return sessionStorage.getItem('limewoodAssetReturn')||'';
}
function closeAsset() {
  const returnTo=assetReturnTarget();
  els.modal.classList.remove('open');
  els.modal.setAttribute('aria-hidden','true');
  document.body.classList.remove('modal-open');
  current=null;
  if(returnTo && returnTo.startsWith('/') && !returnTo.startsWith('//')){
    sessionStorage.removeItem('limewoodAssetReturn');
...
async function uploadPhotos(assetUuid, files) {
  for (const file of files) {
    const uploadToken = (globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`);
    const path = `${assetUuid}/photos/${Date.now()}-${uploadToken}-${cleanName(file.name || 'asset-photo.jpg')}`;
...
async function saveCurrent() {
  ...
  try {
    ...
    if (editing) {
      els.editFields.querySelectorAll('[data-field]').forEach(i => {
        if(['buildingName','areaName','subAreaName'].includes(i.dataset.field)) return;
        current[i.dataset.field]=i.value.trim();
      });
      current.manufacturerUrl=els.manufacturerLink.value.trim(); current.manualUrl=els.manualLink.value.trim();
    }
    if (!current.id) current.id=nextAssetCode();
    if (!current.name) throw new Error('Asset name is required.');
    const location = await resolveAssetLocation(current);
    const payload = {
      asset_code:current.id, asset_name:current.name, building_id:location.building_id, plant_room_id:location.plant_room_id,
      area_id:location.area_id, sub_area_id:location.sub_area_id, exact_location:current.exactLocation||null,
  ...
  try {
    ...
    if(directAsset&&assets.some(a=>a.id===directAsset)){
      try{
        const ref=new URL(document.referrer);
        if(ref.origin===location.origin && ref.pathname && ref.pathname!=='/' && ref.pathname!==location.pathname){
          sessionStorage.setItem('limewoodAssetReturn',ref.pathname+ref.search);
        }
      }catch(_){}
      closeDrawer();
      showRegister('');
...
$('quickMechanical')?.addEventListener('click',()=>showDisciplineAssets('', ['water','plumb','pump','valve','tank','cws','hws','dhw','tmv','booster'],'Mechanical / Plumbing'));
$('quickHeating')?.addEventListener('click',()=>showDisciplineAssets('', ['heating','boiler','lthw','radiator','calorifier','heat exchanger','pressurisation'],'Heating'));
$('quickFire')?.addEventListener('click',()=>showDisciplineAssets('', ['fire','alarm','emergency lighting','sprinkler','smoke','life safety'],'Fire & Life Safety'));
$('quickDocuments')?.addEventListener('click',()=>showDocuments(''));
...
$('closeMaintenanceHistory')?.addEventListener('click', closeMaintenanceHistory);
$('historySearch')?.addEventListener('input', renderMaintenanceHistory);
$('historyTypeFilter')?.addEventListener('change', renderMaintenanceHistory);
$('historyDateFilter')?.addEventListener('change', renderMaintenanceHistory);
$('historyBmsBtn')?.addEventListener('click', e => openBms(e.currentTarget.dataset.bmsKey || 'overview'));
$('maintenanceHistoryModal')?.addEventListener('click', e => {
  if (e.target?.id === 'maintenanceHistoryModal') closeMaintenanceHistory();
});
...
function refreshV6Metrics(){
  ...
  if($('metricDocumentCount'))$('metricDocumentCount').textContent=libraryDocuments.length;
...
function showRegister(room=''){
  ...
  registerForcedRoom='';
  registerDisciplineTerms=[];
  registerDisciplineTitle='';

  showView('register');
  const matchingRoomOption=[...els.room.options].find(o=>samePlantRoom(o.value,room));
  els.room.value=matchingRoomOption?.value || room || '';

  registerTitle.textContent=room
    ? room.replace(/ Plant Room$/i,' Asset Register')
...
function showDisciplineAssets(room,terms,title){
  const canonicalRoom=room?canonicalPlantRoomName(room):'';

  registerForcedRoom=canonicalRoom;
  registerDisciplineTerms=(terms||[]).map(x=>String(x).toLowerCase());
  registerDisciplineTitle=canonicalRoom
    ? `${canonicalRoom.replace(/ Plant Room$/i,'')} · ${title}`
    : title;
  ...
  if(matchingRoomOption){
    ...
    option.value=canonicalRoom;
    option.textContent=canonicalRoom;
    els.room.appendChild(option);
    els.room.value=canonicalRoom;
  }else{
    els.room.value='';
  }
...
// Creator bridge is defined at the top of this file and bound to the
// Log Builder functions inside the main Limewood application scope.