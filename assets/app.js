...
const SESSION_FALLBACK_KEY='limewood-supabase-session-v1';

function saveSessionFallback(s){
  try{
    if(s?.access_token && s?.refresh_token){
      localStorage.setItem(SESSION_FALLBACK_KEY,JSON.stringify({
        access_token:s.access_token,
        refresh_token:s.refresh_token,
        expires_at:s.expires_at||null
      }));
    }
  }catch(e){ console.warn('Session fallback save failed',e); }
}

function clearSessionFallback(){
  try{ localStorage.removeItem(SESSION_FALLBACK_KEY); }catch(e){}
}

async function restoreSessionFallback(){
  try{
    ...
    console.warn('Session fallback restore failed',error);
    clearSessionFallback();
    return null;
  }
}
...
async function signIn() {
  ...
  try {
    ...
    if(!result?.data?.session) throw new Error('Supabase accepted the request but did not return a session.');
    saveSessionFallback(result.data.session);

    els.authMessage.textContent='Signed in. Opening Limewood Engineering…';
...
async function startApp(newSession) {
  ...
  appStarting=true;
  session=newSession;
  saveSessionFallback(newSession);

  // Authentication has succeeded. Do not keep the user trapped on the login
  // screen while secondary cloud modules initialise.
  els.authScreen.hidden=true;
  els.appShell.hidden=false;
  els.authMessage.textContent='';
  updateDashboardGreeting();
  showView('dashboard');
  setSync('Signed in · syncing cloud data…');

  appStarting=false;

  // Run the data bootstrap separately so one slow/failed optional module cannot
  // prevent access to the application shell.
  if(!appBootstrapping){
    appBootstrapping=true;
    bootstrapApp().finally(()=>{ appBootstrapping=false; });
  }
}

function withStartupTimeout(promise,label,ms=20000){
  let timer;
  const timeout=new Promise((_,reject)=>{
...
async function bootstrapApp(){
  ...
  try{
    await withStartupTimeout(loadCloud(),'Initial cloud sync',20000);
    cloudReady=true;
  }catch(error){
    console.error('Initial cloud sync failed:',error);
    setSync(`Signed in · asset sync issue: ${error?.message||error}`,true);
  }

  if(cloudReady){
    try{
      await withStartupTimeout(seedExistingAssets(),'Asset seed',15000);
      await withStartupTimeout(loadCloud(),'Cloud refresh',20000);
    }catch(error){
      console.error('Asset seed/refresh failed:',error);
    }
  }

  // Valves/PPM and navigation are core operational UI, not optional extras.
  try{
    await withStartupTimeout(loadOperations(),'Operations',15000);
  }catch(error){
    console.error('Operations startup failed:',error);
  }
  ...
  try{
    ...
    console.error('Core UI refresh failed:',error);
  }

  // Documents and logs can load later without blocking the usable engineering UI.
  Promise.allSettled([
    withStartupTimeout(loadDocumentCentre(),'Document Centre',15000),
    withStartupTimeout(loadLogs(),'Logs',15000)
  ]).then(results=>{
    results.forEach((result,i)=>{
      if(result.status==='rejected') console.warn(i===0?'Document Centre startup failed:':'Logs startup failed:',result.reason);
    });
  ...
  if(cloudReady) setSync(`Cloud synced · ${assets.length} assets`);
}
...
function stopApp() { session=null; clearSessionFallback(); appStarting=false; appBootstrapping=false; assets=[]; els.appShell.hidden=true; els.authScreen.hidden=false; els.authPassword.value=''; els.authMessage.textContent=''; }
...
client.auth.onAuthStateChange((event,newSession)=>{
  // Backup session handoff. Password sign-in also calls startApp directly.
...
client.auth.getSession().then(async ({data})=>{
  ...
  if(!session){
    const restored=await restoreSessionFallback();
    if(restored) startApp(restored);
    else stopApp();
  }
});
})();
...
if('serviceWorker' in navigator){
  window.addEventListener('load',async()=>{
    try{
      const reg=await navigator.serviceWorker.register('/service-worker.js?v=9990011',{updateViaCache:'none'});
      await reg.update();
