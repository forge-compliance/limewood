...
function normaliseLegacyValve(v,index=0){
 const room=normalisePlantRoomName(v.plant_room||v.plantRoom||v.room||v['Plant room']||'');
 const tag=String(v.tag||v.valve_tag||v.valveTag||v.valve_no||v.valveNo||v['Valve No.']||v['Valve No']||index+1).trim();
 return {
  ...v,
  id:v.id||`local-recovered-${Date.now()}-${index}`,
  tag,
  plant_room:room,
  asset_code:v.asset_code||v.assetCode||null,
  service_duty:v.service_duty||v.service||v.system||'',
  valve_type:v.valve_type||v.type||'',
  size:v.size||v.size_mm||'',
  normal_position:v.normal_position||v.position||'Open',
  last_verified:v.last_verified||v.lastVerified||null,
  location:v.location||'',
  isolation_purpose:v.isolation_purpose||v.function||v.duty||v.description||'',
  notes:v.notes||'',
  updated_at:v.updated_at||new Date().toISOString()
 };
}
function mergeValveRecords(cloudRows=[],localRows=[]){
 const merged=[],seen=new Set();
 [...cloudRows,...localRows].forEach((raw,index)=>{
  const v=normaliseLegacyValve(raw,index),key=`${String(v.plant_room).toLowerCase()}|${String(v.tag).toLowerCase()}`;
  if(!seen.has(key)){seen.add(key);merged.push(v)}
 });
 return merged;
}
async function syncRecoveredValves(localRows,cloudRows){
 if(!operationsCloudReady||!localRows.length)return;
 const cloudKeys=new Set(cloudRows.map(v=>`${String(v.plant_room||'').toLowerCase()}|${String(v.tag||'').toLowerCase()}`));
 const missing=localRows.map(normaliseLegacyValve).filter(v=>!cloudKeys.has(`${String(v.plant_room).toLowerCase()}|${String(v.tag).toLowerCase()}`));
 if(!missing.length)return;
 const payload=missing.map(v=>{const x={...v,updated_by:session.user.id};if(String(x.id).startsWith('local-'))delete x.id;return x});
 const {data,error}=await client.from('valve_register').upsert(payload,{onConflict:'tag'}).select();
 if(error){console.warn('Recovered valve cloud sync deferred:',error.message);saveOpsLocal();return}
 valveRecords=mergeValveRecords(data||[],valveRecords);saveOpsLocal();populateOpsFilters();renderValves();
}
async function loadOperations(){
 const local=readOpsLocal();
 const localPpm=Array.isArray(local.ppmRecords)?local.ppmRecords:[];
 const localValves=Array.isArray(local.valveRecords)?local.valveRecords:[];
 const [p,v]=await Promise.all([client.from('ppm_schedules').select('*'),client.from('valve_register').select('*')]);
 operationsCloudReady=!p.error&&!v.error;
 const cloudPpm=operationsCloudReady?(p.data||[]):[];
 const cloudValves=operationsCloudReady?(v.data||[]):[];
 ppmRecords=operationsCloudReady?(cloudPpm.length?cloudPpm:localPpm):localPpm;
 valveRecords=operationsCloudReady?mergeValveRecords(cloudValves,localValves):localValves.map(normaliseLegacyValve);
 seedPpmFromAssets();saveOpsLocal();populateOpsFilters();renderPpm();renderValves();setTimeout(openValveFromUrl,0);
 if(operationsCloudReady&&localValves.length)syncRecoveredValves(localValves,cloudValves);
}
...
function populateOpsFilters(){
 const rooms=[...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),...valveRecords.map(v=>v.plant_room),...customPlantRooms()].filter(Boolean))].sort((a,b)=>a.localeCompare(b));
...
function renderValveRoomButtons(rooms){
 const grid=$('valveRoomButtons'),select=$('valveRoom');if(!grid||!select)return;
 const current=select.value;
 const all=[{name:'',label:'All plant rooms'},...rooms.map(name=>({name,label:name.replace(/ Plant Room$/i,'')}))];
 grid.innerHTML=all.map(room=>{const count=room.name?valveRecords.filter(v=>v.plant_room===room.name).length:valveRecords.length;return `<button type="button" class="valveRoomCard ${current===room.name?'active':''}" data-valve-room-card="${esc(room.name)}"><span class="roomIcon">${room.name?'🏭':'🌿'}</span><span><b>${esc(room.label)}</b><small>${count} valve${count===1?'':'s'}</small></span></button>`}).join('');
}

function renderPpm(){const body=$('ppmRows');if(!body)return;const q=$('ppmSearch').value.toLowerCase(),room=$('ppmRoom').value,status=$('ppmStatus').value;const rows=ppmRecords.map(r=>({...r,asset:assetForCode(r.asset_code),computed:ppmStatus(r)})).filter(r=>{const hay=[r.asset_code,r.asset?.name,r.asset?.room,r.assigned_to,r.task,r.notes].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!room||r.asset?.room===room)&&(!status||r.computed===status)}).sort((a,b)=>String(a.next_due||'9999').localeCompare(String(b.next_due||'9999')));
 $('ppmTotal').textContent=ppmRecords.length;$('ppmOverdue').textContent=ppmRecords.filter(r=>ppmStatus(r)==='Overdue').length;$('ppmDueSoon').textContent=ppmRecords.filter(r=>ppmStatus(r)==='Due soon').length;$('ppmCompleted').textContent=ppmRecords.filter(r=>ppmStatus(r)==='Complete').length;
 body.innerHTML=rows.length?rows.map(r=>`<tr><td><b>${esc(r.asset_code)}</b><small>${esc(r.asset?.name||'Asset not found')}</small></td><td>${esc(r.asset?.room||'To confirm')}</td><td>${esc(r.frequency||'To confirm')}</td><td>${esc(r.last_completed||'—')}</td><td>${esc(r.next_due||'Set date')}</td><td><span class="opsStatus ${r.computed.toLowerCase().replace(/ /g,'-')}">${esc(r.computed)}</span></td><td>${esc(r.assigned_to||'Unassigned')}</td><td><button data-ppm="${esc(r.asset_code)}">Edit</button></td></tr>`).join(''):'<tr><td colspan="8" class="emptyState">No PPM schedules match the filters.</td></tr>'}
function openPpm(code){const r=ppmRecords.find(x=>x.asset_code===code),a=assetForCode(code);if(!r)return;$('ppmModalTitle').textContent=`${code} · ${a?.name||'PPM schedule'}`;$('ppmAsset').value=code;$('ppmFrequency').value=r.frequency||a?.ppm||'';$('ppmLast').value=r.last_completed||'';$('ppmNext').value=r.next_due||'';$('ppmAssigned').value=r.assigned_to||'';$('ppmCompletion').value=r.completion_status||'Scheduled';$('ppmTask').value=r.task||'';$('ppmNotes').value=r.notes||'';openOpsModal('ppmModal')}
async function savePpmRecord(){const code=$('ppmAsset').value,r=ppmRecords.find(x=>x.asset_code===code);if(!r)return;Object.assign(r,{frequency:$('ppmFrequency').value,last_completed:$('ppmLast').value,next_due:$('ppmNext').value,assigned_to:$('ppmAssigned').value.trim(),completion_status:$('ppmCompletion').value,task:$('ppmTask').value.trim(),notes:$('ppmNotes').value.trim(),updated_at:new Date().toISOString()});if(r.completion_status==='Complete'&&r.last_completed&&!r.next_due)r.next_due=addMonthsIso(r.last_completed,frequencyMonths(r.frequency));if(operationsCloudReady){const payload={asset_code:r.asset_code,frequency:r.frequency,last_completed:r.last_completed||null,next_due:r.next_due||null,assigned_to:r.assigned_to||null,completion_status:r.completion_status,task:r.task||null,notes:r.notes||null,updated_by:session.user.id};const {data,error}=await client.from('ppm_schedules').upsert(payload,{onConflict:'asset_code'}).select().single();if(error)return alert(error.message);Object.assign(r,data)}else saveOpsLocal();closeOpsModal('ppmModal');renderPpm()}
...
function fillValveImportRooms(){
 const knownRooms=[
  'Main House Plant Room','Staff House Plant Room','Coach House Plant Room','Forest Lodges Plant Room',
  'Forest Cottage Plant Room','Spa Plant Room','Pavilion Plant Room','Barn Plant Room'
 ];
 const rooms=[...new Set([
  ...knownRooms,
  ...assets.map(a=>a.room),
...
async function previewValveImport(){
 ...
 try{
  ...
  const existing=new Set(valveRecords.map(v=>String(v.tag).toLowerCase()));
  const duplicates=valveImportRows.filter(v=>existing.has(String(v.tag).toLowerCase())).length;
...
async function confirmValveImport(){
 ...
 const button=$('confirmValveImport'),mode=$('valveImportDuplicates').value,existingByTag=new Map(valveRecords.map(v=>[String(v.tag).toLowerCase(),v]));
 const rows=valveImportRows.map((v,i)=>({...v,id:`local-import-${Date.now()}-${i}`,updated_at:new Date().toISOString()}));
 const newRows=rows.filter(v=>!existingByTag.has(String(v.tag).toLowerCase()));
 const updateRows=mode==='update'?rows.filter(v=>existingByTag.has(String(v.tag).toLowerCase())):[];
 ...
 try{
  if(operationsCloudReady){
   ...
   if(payloadRows.length){const {data,error}=await client.from('valve_register').upsert(payloadRows,{onConflict:'tag'}).select();if(error)throw error;for(const saved of data||[]){const old=valveRecords.find(v=>String(v.tag).toLowerCase()===String(saved.tag).toLowerCase());if(old)Object.assign(old,saved);else valveRecords.push(saved)}}
  }else{
   for(const v of newRows)valveRecords.push(v);
   for(const v of updateRows)Object.assign(existingByTag.get(String(v.tag).toLowerCase()),v,{id:existingByTag.get(String(v.tag).toLowerCase()).id});
   saveOpsLocal();
  }
...
function filteredValveRows(){const q=$('valveSearch').value.toLowerCase(),room=$('valveRoom').value,pos=$('valvePosition').value;return valveRecords.filter(v=>{const hay=[v.tag,v.plant_room,v.service_duty,v.valve_type,v.size,v.location,v.isolation_purpose,v.notes,v.asset_code].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!room||v.plant_room===room)&&(!pos||v.normal_position===pos)}).sort((a,b)=>valveSortNumber(a)-valveSortNumber(b)||String(a.tag).localeCompare(String(b.tag)))}
...
function renderValves(){const body=$('valveRows');if(!body)return;const rows=filteredValveRows();
 renderValveRoomButtons([...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),...valveRecords.map(v=>v.plant_room),...customPlantRooms()].filter(Boolean))].sort((a,b)=>a.localeCompare(b)));
...
function openValveFromUrl(){const params=new URLSearchParams(location.search),key=params.get('valve');if(!key)return;const room=params.get('room')||'',v=valveRecords.find(x=>String(x.tag)===key||String(x.id)===key);if(v){showView('valves');if(room&&[...$('valveRoom').options].some(o=>o.value===room))$('valveRoom').value=room;renderValves();openValveDetail(v.id)}}
function openValve(id=''){editingValveId=id;const v=valveRecords.find(x=>String(x.id)===String(id))||{};$('valveModalTitle').textContent=id?'Edit valve':'Add valve';$('vTag').value=v.tag||'';$('vRoom').value=v.plant_room||$('vRoom').options[0]?.value||'';$('vAsset').value=v.asset_code||'';$('vService').value=v.service_duty||'';$('vType').value=v.valve_type||'';$('vSize').value=v.size||'';$('vPosition').value=v.normal_position||'Open';$('vVerified').value=v.last_verified||'';$('vLocation').value=v.location||'';$('vIsolation').value=v.isolation_purpose||'';$('vNotes').value=v.notes||'';$('deleteValve').hidden=!id;openOpsModal('valveModal')}
...
els.signIn.onclick=signIn; els.signUp.onclick=signUp; els.signOut.onclick=()=>client.auth.signOut(); els.refresh.onclick=()=>Promise.all([loadCloud(),loadDocumentCentre(),loadOperations()]).catch(e=>{setSync(e.message,true);alert(e.message)});
...
const runGlobalSearch=()=>{const q=els.globalSearch?.value.trim()||''; showRegister(''); els.search.value=q; render();};
...
$('quickEstateRegister')?.addEventListener('click',()=>showRegister(''));
$('quickPpm')?.addEventListener('click',()=>{showView('ppm');renderPpm()});
$('quickValves')?.addEventListener('click',()=>{showView('valves');renderValves()});
...
const dashboardView=$('dashboardView'),plantRoomHubView=$('plantRoomHubView'),registerView=$('registerView'),documentView=$('documentView'),complianceView=$('complianceView'),ppmView=$('ppmView'),valveView=$('valveView'),placeholderView=$('placeholderView'),registerTitle=$('registerTitle'),drawer=$('drawer'),backdrop=$('drawerBackdrop');
let currentHubRoom='';
function closeDrawer(){drawer.classList.remove('open');backdrop.classList.remove('open');drawer.setAttribute('aria-hidden','true')}
function showView(n){dashboardView.hidden=n!=='dashboard';plantRoomHubView.hidden=n!=='plantRoomHub';registerView.hidden=n!=='register';documentView.hidden=n!=='documents';complianceView.hidden=n!=='compliance';ppmView.hidden=n!=='ppm';valveView.hidden=n!=='valves';$('bmsView').hidden=n!=='bms';placeholderView.hidden=n!=='placeholder'}

function allPlantRoomNames(){return [...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),...valveRecords.map(v=>v.plant_room),...customPlantRooms()].filter(Boolean))].sort((a,b)=>a.localeCompare(b));}
...
function showPlantRoomDirectory(){const rooms=allPlantRoomNames();const card=placeholderView.querySelector('.placeholderCard');card.innerHTML=`<span>PLANT ROOMS</span><h2>Select a plant room</h2><p>Choose a plant room to open its engineering hub.</p><div class="plantHubGrid directoryGrid">${rooms.length?rooms.map(r=>`<button data-select-plant-room="${esc(r)}"><span>🏭</span><b>${esc(r.replace(/ Plant Room$/i,''))}</b><small>${assets.filter(a=>a.room===r).length} assets · ${valveRecords.filter(v=>v.plant_room===r).length} valves</small></button>`).join(''):'<p class="emptyState">No plant rooms have been added yet.</p>'}</div><button id="roomDirectoryBack">Return to dashboard</button>`;showView('placeholder');closeDrawer();}
function showAssetRegisterDirectory(){const rooms=allPlantRoomNames();const card=placeholderView.querySelector('.placeholderCard');card.innerHTML=`<span>ASSET REGISTERS</span><h2>Select an asset register</h2><p>Open the estate register or choose a plant room.</p><div class="plantHubGrid directoryGrid"><button data-select-asset-room=""><span>📋</span><b>Estate Register</b><small>${assets.length} assets across all rooms</small></button>${rooms.map(r=>`<button data-select-asset-room="${esc(r)}"><span>📋</span><b>${esc(r.replace(/ Plant Room$/i,''))}</b><small>${assets.filter(a=>a.room===r).length} assets</small></button>`).join('')}</div><button id="assetDirectoryBack">Return to dashboard</button>`;showView('placeholder');closeDrawer();}
function roomDocumentCount(room){const pr=plantRooms.find(r=>r.name===room);return libraryDocuments.filter(d=>String(d.plantRoomId||'')===String(pr?.id||'')||String(d.title||'').toLowerCase().includes(room.replace(/ Plant Room$/,'').toLowerCase())).length;}
function showPlantRoomHub(room){currentHubRoom=room||'Unassigned Plant Room';showView('plantRoomHub');$('hubRoomTitle').textContent=currentHubRoom;$('hubAssetCount').textContent=assets.filter(a=>a.room===currentHubRoom).length;$('hubValveCount').textContent=valveRecords.filter(v=>v.plant_room===currentHubRoom).length;$('hubPpmCount').textContent=ppmRecords.filter(p=>p.plant_room===currentHubRoom).length;$('hubDocumentCount').textContent=roomDocumentCount(currentHubRoom);$('hubRoomSummary').textContent='Choose assets, valves, PPM, QR labels, documents, compliance or live BMS for this room.';$('hubSearch').value='';closeDrawer();}
function printRoomQrLabels(room){if(typeof QRCode==='undefined')return alert('QR library could not load. Reload while connected to the internet.');const list=assets.filter(a=>a.room===room);if(!list.length)return alert('There are no assets in this plant room yet.');const w=open('','_blank');w.document.write('<title>'+esc(room)+' QR Labels</title><style>@page{size:A4;margin:8mm}body{font-family:Arial}.sheet{display:grid;grid-template-columns:repeat(3,1fr);gap:6mm}.label{border:1px solid #333;padding:8px;text-align:center;break-inside:avoid}.qr{width:110px;height:110px;margin:auto}.qr canvas,.qr img{width:110px!important;height:110px!important}h3{font-size:14px;margin:5px 0 2px}p{font-size:9px;margin:2px}</style><h2>'+esc(room)+'</h2><div class="sheet" id="sheet"></div>');const sheet=w.document.getElementById('sheet');list.forEach(a=>{const d=w.document.createElement('div');d.className='label';d.innerHTML=`<div class="qr"></div><h3>${esc(a.id)}</h3><p><b>${esc(a.name)}</b></p><p>${esc(a.room)}</p>`;sheet.appendChild(d);new QRCode(d.querySelector('.qr'),{text:assetUrl(a.id),width:110,height:110,correctLevel:QRCode.CorrectLevel.M});});w.document.write('<script>setTimeout(()=>print(),900)<\/script>');w.document.close();}
function openHubDocuments(type=''){showDocuments(type);const pr=plantRooms.find(r=>r.name===currentHubRoom);if(pr){const building=buildings.find(b=>b.id===pr.building_id);if(building){docEls.buildingFilter.value=building.id;renderDocuments();}}}
...
function placeholder(t){const card=placeholderView.querySelector('.placeholderCard');card.innerHTML=`<span>SECTION READY</span><h2 id="placeholderTitle">${esc(t)}</h2><p>This section is included and ready for information as surveys and documents are completed.</p><button id="backDashboard">Return to dashboard</button>`;showView('placeholder');closeDrawer()}
...
$('plantRoomsMenu')?.querySelector('summary')?.addEventListener('click',e=>{e.preventDefault();$('plantRoomsMenu').open=false;showPlantRoomDirectory();});
...
$('placeholderView')?.addEventListener('click',e=>{const room=e.target.closest('[data-select-plant-room]');if(room)return showPlantRoomHub(room.dataset.selectPlantRoom);const assetRoom=e.target.closest('[data-select-asset-room]');if(assetRoom)return showRegister(assetRoom.dataset.selectAssetRoom||'');if(e.target.closest('#roomDirectoryBack,#assetDirectoryBack,#backDashboard'))showView('dashboard');});
$('drawerNav').onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.hubRoom){showPlantRoomHub(b.dataset.hubRoom)}else if(b.dataset.view==='dashboard'){showView('dashboard');closeDrawer()}else if(b.dataset.view==='compliance'){showCompliance()}else if(b.dataset.view==='bms'){showView('bms');closeDrawer()}else if(b.dataset.view==='ppm'){showView('ppm');renderPpm();closeDrawer()}else if(b.dataset.view==='valves'){showView('valves');renderValves();closeDrawer()}else if(b.dataset.view==='search'){showRegister('');setTimeout(()=>els.search.focus(),200)}else if(b.dataset.view==='documents'){showDocuments(b.dataset.docType||'')}else if('room'in b.dataset)showPlantRoomHub(b.dataset.room||'');else if(b.dataset.placeholder)placeholder(b.dataset.placeholder)};
...
['ppmSearch','ppmRoom','ppmStatus'].forEach(id=>$(id)?.addEventListener('input',renderPpm));
['valveSearch','valveRoom','valvePosition'].forEach(id=>$(id)?.addEventListener('input',renderValves));
$('valveRoomButtons')?.addEventListener('click',e=>{const b=e.target.closest('[data-valve-room-card]');if(!b)return;$('valveRoom').value=b.dataset.valveRoomCard||'';renderValves();});
