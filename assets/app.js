...
let roomDiscipline = ''; // Optional plant-room discipline filter
...
function filtered() {
  ...
  return assets.filter(a => {
    ...
    const disciplineHay=[a.category,a.system,a.name,a.notes].join(' ').toLowerCase();
    const disciplineTerms={
      plumbing:['plumb','water','cws','hws','dhw','domestic','tmv','cold water','hot water','water treatment','softener'],
      heating:['heating','lthw','boiler','calorifier','heat','radiator','pressurisation','expansion vessel','primary pump'],
      fire:['fire','life safety','emergency light','alarm','sprinkler','smoke','heat detector']
    };
    const disciplineMatch=!roomDiscipline||(disciplineTerms[roomDiscipline]||[]).some(t=>disciplineHay.includes(t));
    return disciplineMatch && (!q || hay.includes(q)) && (!els.room.value || samePlantRoom(a.room,els.room.value)) && (!els.category.value || a.category === els.category.value) && (!els.completeness.value || (els.completeness.value === 'missing' ? isMissing(a) : !isMissing(a)));
  });
}
...
function showRegister(room=''){roomDiscipline='';room=room?canonicalPlantRoomName(room):'';showView('register');els.room.value=room;registerTitle.textContent=room?room.replace(' Plant Room',' Asset Register'):'Estate Asset Register';render();closeDrawer()}
function showRoomDiscipline(room,discipline,label){
  roomDiscipline=discipline; room=canonicalPlantRoomName(room); showView('register'); els.room.value=room; els.category.value=''; els.completeness.value=''; els.search.value='';
  registerTitle.textContent=`${room.replace(/ Plant Room$/i,'')} · ${label}`; render(); closeDrawer();
}
function openRoomElectrical(room){location.href=`/electrical-distribution.html?room=${encodeURIComponent(canonicalPlantRoomName(room))}`;}
...
$('plantRoomHubView')?.addEventListener('click',e=>{const b=e.target.closest('[data-hub-action]');if(!b)return;const a=b.dataset.hubAction;if(a==='electrical')openRoomElectrical(currentHubRoom);else if(a==='plumbing')showRoomDiscipline(currentHubRoom,'plumbing','Plumbing & Water');else if(a==='heating')showRoomDiscipline(currentHubRoom,'heating','Heating');else if(a==='fire')showRoomDiscipline(currentHubRoom,'fire','Fire & Life Safety');else if(a==='assets')showRegister(currentHubRoom);else if(a==='valves'){showView('valves');openValveRoomRegister(currentHubRoom)}else if(a==='ppm'){openPpmRoom(currentHubRoom)}else if(a==='qr')printRoomQrLabels(currentHubRoom);else if(['sop','rams','manual','schematic'].includes(a))openHubDocuments(a);else if(a==='photos'){showRegister(currentHubRoom);els.search.value='';render()}else if(a==='logs'){showLogsHome();const q=$('logHistorySearch');if(q)q.value=currentHubRoom.replace(/ Plant Room$/i,'');}else if(a==='compliance')showCompliance();else if(a==='bms'){showView('bms')}});
