...
const els = {
  ...
  modalDetails:$('mDetails'), modalNotes:$('mNotes'), modalStatus:$('mStatus'), save:$('saveBtn'), close:$('closeAssetModal'),
...
// This prevents a later render() from silently falling back to another room.
...
function filtered() {
  ...
  const disciplineTerms = registerDisciplineTerms.map(x=>String(x).toLowerCase());

  return assets.filter(a => {
    const hay = [a.id,a.name,a.room,a.category,a.system,a.serviceDuty,a.manufacturer,a.model,a.serial,a.notes]
      .join(' ')
      .toLowerCase();

    const roomMatches = !effectiveRoom || samePlantRoom(a.room,effectiveRoom);
    const disciplineMatches = !disciplineTerms.length || disciplineTerms.some(term=>hay.includes(term));

    return (!q || hay.includes(q))
      && roomMatches
      && disciplineMatches
      && (!els.category.value || a.category === els.category.value)
      && (!els.completeness.value || (els.completeness.value === 'missing' ? isMissing(a) : !isMissing(a)));
  });
}
...
function render() {
  ...
  if(registerDisciplineTitle) registerTitle.textContent = registerDisciplineTitle;
  els.grid.innerHTML = visibleRows.map(a => `<article class="card assetCompactCard assetTextTile" tabindex="0" data-id="${esc(a.id)}">
    <div class="cardBody">
      <div class="topline">
      ...
      <h4>${esc(a.name)}</h4>
      <div class="assetTileRoom">${esc(a.room || 'Plant room not set')}</div>
      <div class="assetTileMeta">
        <span><small>System</small><b>${esc(a.system || 'To confirm')}</b></span>
        <span><small>Category</small><b>${esc(a.category || 'Uncategorised')}</b></span>
      </div>
      <div class="assetTileFooter">
        <span class="criticalityPill">${esc(a.criticality || 'Medium')} criticality</span>
        <span class="assetTileOpen">Open →</span>
      </div>
    </div>
...
function historyRecordSearchText(record) {
  return [
    record.work_date, record.work_type, record.description, record.engineer_name,
    record.findings, record.actions_taken, record.follow_up_date
...
function renderMaintenanceHistory() {
  ...
  const type = String($('historyTypeFilter')?.value || '');
  const days = String($('historyDateFilter')?.value || 'all');

  let records = [...maintenanceHistoryRecords];

  if (search) records = records.filter(r => historyRecordSearchText(r).includes(search));
  if (type) records = records.filter(r => String(r.work_type || 'Maintenance') === type);

  if (days !== 'all') {
    const cutoff = Date.now() - (Number(days) * 86400000);
    records = records.filter(r => {
      const t = r.work_date ? new Date(`${String(r.work_date).slice(0,10)}T12:00:00`).getTime() : 0;
      return t >= cutoff;
    });
  }

  const resultMeta = $('historyResultMeta');
  if (resultMeta) resultMeta.textContent = `${records.length} of ${maintenanceHistoryRecords.length} record${maintenanceHistoryRecords.length === 1 ? '' : 's'}`;

  if (!records.length) {
    list.innerHTML = `<div class="historyEmpty"><b>No matching maintenance records</b><span>Try changing the search or filters.</span></div>`;
    return;
  }
  ...
  list.innerHTML = records.map((record, index) => {
    ...
    return `
      <details class="historyRecord${followUp ? ' needsFollowUp' : ''}"${open}>
        <summary>
          <span class="historyTimelineDot" aria-hidden="true"></span>
          ...
          <span class="historyStatus ${followUp ? 'followup' : 'complete'}">${followUp ? 'FOLLOW-UP' : 'COMPLETE'}</span>
          <span class="historyChevron" aria-hidden="true">⌄</span>
        </summary>
