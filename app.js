// ===== Limewood Creator global bridge =====
// This deliberately lives OUTSIDE the main app wrapper so Android WebView,
// Chrome and the live engineering app all see the same stable API.
...
// Creator commands are received here, inside the application scope.
// Function declarations below are hoisted, so these handlers can be
// registered before the rest of the app finishes initialising.
...
window.LIMEWOOD_CREATOR.ready = true;
...
function ensureCreatorLogBuilder(){
  ...
  style.textContent=`
    ...
    @media(max-width:780px){#creatorLogBuilder{padding:0}.clbShell{border-radius:0;min-height:100vh}.clbBody{grid-template-columns:1fr}.clbList{border-right:0;border-bottom:1px solid rgba(23,61,51,.14);max-height:220px;overflow:auto}.clbGrid,.clbFieldTop,.clbFieldMore{grid-template-columns:1fr}.clbEditor{padding:14px}}
  `;
