...
async function loadCloud() {
  ...
  // Keep photo metadata only at initial sync. Do NOT create/load every Supabase image on the register page.
  // Full-resolution photos are signed and fetched only when an individual asset is opened.
  ...
  assets = (aRes.data || []).map(row => toView({
    ...row,
    photos: photos.filter(p => p.asset_id === row.id),
...
async function renderGallery() {
  const assetAtStart = current;
  const cloud = assetAtStart?.photos || [];
  const local = localPhoto(assetAtStart || {});

  // Show the local image immediately while cloud photo URLs are generated on demand.
  const localItems = local.includes('placeholder') ? [] : [{src:local,caption:'Original survey photograph'}];
  if (cloud.length) {
    els.gallery.innerHTML = localItems.length
      ? localItems.map(x => `<figure><img loading="lazy" decoding="async" src="${esc(x.src)}" alt="${esc(x.caption)}"><figcaption>${esc(x.caption)}</figcaption></figure>`).join('') + '<p class="galleryLoading">Loading uploaded photographs…</p>'
      : '<p class="galleryLoading">Loading uploaded photographs…</p>';
  } else {
    els.gallery.innerHTML = localItems.length
      ? localItems.map(x => `<figure><img loading="lazy" decoding="async" src="${esc(x.src)}" alt="${esc(x.caption)}"><figcaption>${esc(x.caption)}</figcaption></figure>`).join('')
      : '<figure><img src="assets/images/asset-placeholder.svg" alt="No photograph"><figcaption>No photograph uploaded</figcaption></figure>';
    return;
  }

  const cloudItems = (await Promise.all(cloud.map(async p => {
    const url = p.url || (p.storage_path ? await signedUrl(p.storage_path) : '');
    return url ? {src:url, caption:p.caption || p.photo_type || 'Asset photograph'} : null;
  }))).filter(Boolean);

  // The user may have opened another asset while URLs were being created.
  if (current !== assetAtStart) return;
  const items = [...cloudItems, ...localItems];
  els.gallery.innerHTML = items.length
    ? items.map(x => `<figure><img loading="lazy" decoding="async" src="${esc(x.src)}" alt="${esc(x.caption)}"><figcaption>${esc(x.caption)}</figcaption></figure>`).join('')
    : '<figure><img src="assets/images/asset-placeholder.svg" alt="No photograph"><figcaption>No photograph uploaded</figcaption></figure>';
}
...
async function signIn() {
  ...
  try {
    ...
    if(result?.data?.session && !session) await startApp(result.data.session);
  } catch(error) {
    clearTimeout(timeoutId);
...
async function startApp(newSession) {
  ...
  if(session && els.appShell && !els.appShell.hidden) return;
  appStarting=true;
  session=newSession;
...
// Do not call Supabase-backed startup work directly inside the auth callback.
// Deferring it avoids auth-lock deadlocks in supabase-js during sign-in/token refresh.
client.auth.onAuthStateChange((event,newSession)=>{
  setTimeout(()=>{
    if(newSession&&!session) startApp(newSession);
    else if(!newSession&&session) stopApp();
  },0);
});
