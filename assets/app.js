...
function fillValveImportRooms(){
 const rooms=[...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),'Forest Cottage Plant Room'].filter(Boolean))].sort();
 const select=$('valveImportRoom');if(!select)return;select.innerHTML=rooms.map(r=>`<option>${esc(r)}</option>`).join('');
 if(rooms.includes('Forest Cottage Plant Room'))select.value='Forest Cottage Plant Room';
}
