...
async function loadDocumentCentre() {
  const [docRes, sopRes, revRes, linkRes] = await Promise.all([
    client.from('asset_documents').select('*').order('created_at', {ascending:false}),
  ...
  const error = docRes.error || sopRes.error || revRes.error || linkRes.error;
  if (error) throw error;
  const revisions = revRes.data || [];
  const links = linkRes.data || [];
  const ordinary = await Promise.all((docRes.data || []).map(async d => ({
    key:`document-${d.id}`, source:'asset_documents', id:d.id, type:d.document_type || 'other', number:'', title:d.title,
    category:'', revision:d.revision || '', status:'approved', buildingId:'', plantRoomId:'', assetIds:d.asset_id ? [d.asset_id] : [],
    createdAt:d.created_at, url:d.external_url || (d.storage_path ? await signedUrl(d.storage_path) : ''), raw:d
  })));
  const sops = await Promise.all((sopRes.data || []).map(async row => {
...
function documentTypeLabel(type) {
  return ({sop:'SOP',rams:'RAMS',manual:'Manual',isolation_procedure:'Isolation procedure',schematic:'Schematic',certificate:'Certificate',report:'Report',other:'Other'})[type] || type || 'Document';
}
...
async function saveLibraryDocument() {
  try {
    ...
    docEls.save.disabled=true; docEls.save.textContent='Uploading…'; docEls.message.textContent='Uploading file and saving record…';
    if(type==='sop') {
      const number=docEls.number.value.trim();
      if(!number) throw new Error('An SOP number is required.');
      const {data:sop,error:sopError}=await client.from('sops').insert({
        sop_number:number,title,category:docEls.category.value.trim()||'General',description:docEls.description.value.trim()||null,
        revision:docEls.revision.value.trim()||'1',status:docEls.status.value,author:docEls.author.value.trim()||null,
        approved_by:docEls.approvedBy.value.trim()||null,issue_date:docEls.issueDate.value||null,review_date:docEls.reviewDate.value||null,
        building_id:docEls.building.value||null,plant_room_id:docEls.plantRoom.value||null,created_by:session.user.id,updated_by:session.user.id
      }).select('*').single();
      if(sopError) throw sopError;
      const path=`sops/${sop.id}/${cleanName(docEls.revision.value.trim()||'1')}-${Date.now()}-${cleanName(file.name)}`;
      await uploadLibraryFile(path,file);
      const {error:revError}=await client.from('sop_revisions').insert({sop_id:sop.id,revision:docEls.revision.value.trim()||'1',file_path:path,file_name:file.name,file_type:file.type||null,change_summary:docEls.changeSummary.value.trim()||null,uploaded_by:session.user.id});
      if(revError) throw revError;
      const {error:updateError}=await client.from('sops').update({current_file_path:path,current_file_name:file.name,current_file_type:file.type||null}).eq('id',sop.id);
      if(updateError) throw updateError;
      if(docEls.asset.value) { const {error:linkError}=await client.from('asset_sops').insert({asset_id:docEls.asset.value,sop_id:sop.id}); if(linkError) throw linkError; }
    } else {
      const allowed=['manual','isolation_procedure','rams','schematic','certificate','report','other'];
      const dbType=allowed.includes(type)?type:'other';
      const folder=`documents/${dbType}/${crypto.randomUUID()}`;
      const path=`${folder}/${Date.now()}-${cleanName(file.name)}`;
      await uploadLibraryFile(path,file);
      const decoratedTitle=[docEls.number.value.trim(),title].filter(Boolean).join(' · ');
      const {error}=await client.from('asset_documents').insert({asset_id:docEls.asset.value||null,title:decoratedTitle,document_type:dbType,storage_path:path,revision:docEls.revision.value.trim()||null,uploaded_by:session.user.id});
      if(error) throw error;
    }
    await loadCloud(); await loadDocumentCentre(); closeDocumentUpload(); showDocuments(documentTypeContext); alert('Document uploaded to the shared Document Centre.');
  } catch(error) { console.error(error); docEls.message.textContent=error.message||'Upload failed.'; }
  finally { docEls.save.disabled=false; docEls.save.textContent='Upload to Document Centre'; }
}
...
docEls.building.onchange=updateDocumentPlantRooms; [docEls.search,docEls.typeFilter,docEls.buildingFilter,docEls.statusFilter].forEach(x=>x.addEventListener('input',renderDocuments));
