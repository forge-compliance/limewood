...
function canonicalPlantRoomName(name){
 return normalisePlantRoomName(name);
}
...
function ppmRooms(){
  ...
  ppmRecords.forEach(r=>{const room=r.plant_room||assetForCode(r.asset_code)?.room;if(room)rooms.add(canonicalPlantRoomName(room))});
...
function renderPpmDirectory(){
  ...
  host.innerHTML=rooms.length?rooms.map(room=>{
    const count=ppmRecords.filter(r=>samePlantRoom(r.plant_room||assetForCode(r.asset_code)?.room,room)).length;
...
function allPlantRoomNames(){return [...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),...valveRecords.map(v=>v.plant_room),...customPlantRooms()].filter(Boolean).map(canonicalPlantRoomName))].sort((a,b)=>a.localeCompare(b));}
...
function showAboutPage(){
  const card=placeholderView.querySelector('.placeholderCard');
  card.innerHTML=`<span>ABOUT</span><h2>Limewood Engineering</h2><p>Engineering Control Centre</p><div class="aboutForge"><b>Powered by Forge Compliance</b><span>Built and managed by Forge Compliance Ltd.</span><small>Version 6.0.1</small></div><button id="aboutBack">Return to dashboard</button>`;
  showView('placeholder');closeDrawer();
}
