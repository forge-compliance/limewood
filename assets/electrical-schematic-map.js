...
const building=(br.data||[]).find(b=>b.name===requested)||(br.data||[])[0];if(!building){status.textContent='No building found.';return}
...
const groups=[take('db','Distribution Boards',a=>isDB(a)),take('cu','Consumer Units',a=>isConsumer(a)),take('ilight','iLight Units',a=>isILight(a)),take('dimmers','Dimmer Racks & Lighting Control',a=>isDimmerRack(a)),take('mcp','Motor Control Panels',a=>isMCP(a)),take('emergency','Emergency Lighting',a=>isEmergency(a)),take('fire','Fire & Damper Controls',a=>isFire(a)),take('controls','Equipment Control Panels',a=>isControl(a)),take('main','Main Switchboards',a=>isMain(a)),take('other','Other Electrical Equipment',a=>true)].filter(g=>g.items.length);
const directLimit=4,directGroups=groups.filter(g=>g.items.length<=directLimit),collapsedGroups=groups.filter(g=>g.items.length>directLimit),topBranches=[...directGroups.flatMap(g=>g.items.map(a=>({type:'asset',a}))),...collapsedGroups.map(g=>({type:'group',g}))];
...
let html=`<div class="sourceWrap"><div class="sourceNode"><div class="code">${esc(sourceCode)}</div><h3>${esc(sourceTitle)}</h3><p>${esc(sourceSub)}</p></div></div><div class="trunk"></div><div class="bus"></div><div class="branchGrid">${topBranches.map(x=>x.type==='asset'?itemCard(x.a,true):groupCard(x.g)).join('')}</div>`;
html+=collapsedGroups.map(g=>`<div id="group-${g.key}" class="groupPanel"><div class="groupTitle">${esc(g.title)}</div><div class="itemGrid">${g.items.map(a=>itemCard(a,false)).join('')}</div></div>`).join('');html+='<div id="noMatches" class="noMatches">No electrical items match that search.</div>';canvas.innerHTML=html;
