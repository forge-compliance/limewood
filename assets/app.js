...
function showMaintenanceIssues(){
  ...
  loadMaintenanceIssues().catch(e=>{
    console.warn('Maintenance dashboard data could not load',e);
    renderMaintenanceIssues();
  });
}
...
/* v7.1.3 Maintenance dashboard controls */
['issueSearch','issueStatusFilter','issuePriorityFilter','issueEngineerFilter'].forEach(id=>{
  $(id)?.addEventListener('input',renderMaintenanceIssues);
  $(id)?.addEventListener('change',renderMaintenanceIssues);
});
$('maintenanceIssueCards')?.addEventListener('click',e=>{const b=e.target.closest('[data-issue-id]');if(b)openMaintenanceIssue(b.dataset.issueId)});
$('closeMaintenanceIssueModal')?.addEventListener('click',()=>closeOpsModal('maintenanceIssueModal'));
$('saveMaintenanceIssue')?.addEventListener('click',()=>saveMaintenanceIssueUpdate(false));
