...
$('markChecked').onclick=()=>updateJob({checked_at:new Date().toISOString(),checked_by:session.user.id},`Checked by ${profileName}`,'checked');
...
$('completeJob').onclick=async()=>{if(!selected.checked_at)return toast('Check the job before completing it.');const note=$('newNote').value.trim();if(!note)return toast('Add a completion note first.');await updateJob({status:'completed',completed_at:new Date().toISOString(),completed_by:session.user.id},`Completed: ${note}`,'completed')};
