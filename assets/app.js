...
function showDisciplineAssets(room,terms,title){
  ...
  visibleRows=assets.filter(a=>(!room||samePlantRoom(a.room,room))&&wanted.some(t=>[a.category,a.system,a.serviceDuty,a.name,a.notes].join(' ').toLowerCase().includes(t)));
  registerTitle.textContent=room?`${room.replace(/ Plant Room$/i,'')} · ${title}`:title;
  els.resultCount.textContent=`${visibleRows.length} asset${visibleRows.length===1?'':'s'}`;
  els.grid.innerHTML=visibleRows.length?visibleRows.map(a=>`<article class="card" tabindex="0" data-id="${esc(a.id)}"><img src="${esc(cardImage(a))}" alt="${esc(a.name)}"><div class="cardBody"><div class="topline"><span class="badge">${esc(a.id)}</span><span class="status">${esc(a.status)}</span></div><h4>${esc(a.name)}</h4><div class="meta">${esc(a.category||'')} · ${esc(a.room)}</div></div></article>`).join(''):'<p class="emptyState">No matching assets recorded yet.</p>';
}
