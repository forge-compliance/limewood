...
const ROLE_RANK={engineer:1,supervisor:2,administrator:3};
let currentRole='engineer';
function normalizeRole(value=''){
  const v=String(value||'').trim().toLowerCase();
  if(['administrator','admin','owner'].includes(v))return 'administrator';
  if(['supervisor','manager','lead'].includes(v))return 'supervisor';
  return 'engineer';
}
function hasRole(minimum='engineer'){return (ROLE_RANK[currentRole]||1)>=(ROLE_RANK[minimum]||1)}
function requireRole(minimum,action='perform this action'){
  if(hasRole(minimum))return true;
  alert(`Your ${currentRole} account cannot ${action}. ${minimum==='administrator'?'Administrator':'Supervisor'} access is required.`);
  return false;
}
async function resolveCurrentRole(){
  const user=session?.user;
  if(!user)return 'engineer';
  const metadataRole=user.app_metadata?.role||user.app_metadata?.user_role||user.user_metadata?.role||user.user_metadata?.user_role;
  if(metadataRole)return normalizeRole(metadataRole);
  for(const table of ['profiles','user_profiles']){
    try{
      const {data,error}=await client.from(table).select('role').eq('id',user.id).maybeSingle();
      if(!error&&data?.role)return normalizeRole(data.role);
    }catch(_){/* Optional profile tables are not required. */}
  }
  return 'engineer';
}
function applyRolePermissions(){
  const badge=$('userRoleBadge');
  if(badge){badge.hidden=false;badge.textContent=currentRole[0].toUpperCase()+currentRole.slice(1);badge.dataset.role=currentRole;}
  const show=(id,allowed)=>{const el=$(id);if(el)el.hidden=!allowed};
  // QR creation and printing deliberately remain available to every signed-in engineer.
  ['assetQrBtn','printAllQr'].forEach(id=>show(id,true));
  // Supervisors and administrators maintain assets, valves and bulk imports.
  ['addAssetBtn','editBtn','addValve','importValves','saveValve','deleteValve'].forEach(id=>show(id,hasRole('supervisor')));
  // Estate structure and role administration belong to administrators only.
  ['addImportPlantRoom','addValvePlantRoom','adminNav'].forEach(id=>show(id,hasRole('administrator')));
  document.querySelectorAll('.adminOnly').forEach(el=>el.hidden=!hasRole('administrator'));
}
...
async function loadOperations(){
 ...
 ppmRecords=operationsCloudReady?(p.data||[]):(local.ppmRecords||[]);
 valveRecords=operationsCloudReady?(v.data||[]):(local.valveRecords||[]);
 seedPpmFromAssets();populateOpsFilters();renderPpm();renderValves();
}
...
function saveCustomPlantRoom(name){
 const rooms=[...new Set([...customPlantRooms(),name].map(x=>String(x||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
 localStorage.setItem(CUSTOM_PLANT_ROOMS_KEY,JSON.stringify(rooms));
}
async function addPlantRoomFromUi(targetSelectId){
 if(!requireRole('administrator','add plant rooms'))return;
 ...
 if(!/plant room$/i.test(name))name+=' Plant Room';
 try{
  if(operationsCloudReady)await ensurePlantRoom(name);
  saveCustomPlantRoom(name);
  fillValveImportRooms();populateOpsFilters();
  const target=$(targetSelectId);if(target){target.value=name;target.dispatchEvent(new Event('change'))}
  alert(`${name} added.`);
 }catch(error){
  console.error(error);
  saveCustomPlantRoom(name);fillValveImportRooms();populateOpsFilters();
  const target=$(targetSelectId);if(target)target.value=name;
  alert(`${name} was added on this device. Cloud sync was unavailable: ${error.message}`);
 }
}
...
function resetValveImport(){
 valveImportRows=[];$('valveImportFile').value='';$('valveImportSummary').textContent='No file selected.';$('valveImportPreview').innerHTML='<tr><td colspan="5" class="emptyState">Select a CSV to preview it.</td></tr>';$('valveImportMessage').textContent='';$('confirmValveImport').disabled=true;
}
...
async function previewValveImport(){
 ...
 try{
  const matrix=parseCsv(await file.text());valveImportRows=mapValveCsvRows(matrix,$('valveImportRoom').value);
 ...
 }catch(error){valveImportRows=[];$('confirmValveImport').disabled=true;$('valveImportSummary').textContent='Import preview failed.';$('valveImportPreview').innerHTML='<tr><td colspan="5" class="emptyState">Check the CSV headings and try again.</td></tr>';$('valveImportMessage').textContent=error.message}
}
async function confirmValveImport(){if(!requireRole('supervisor','import valve registers'))return;
...
function renderValves(){const body=$('valveRows');if(!body)return;const q=$('valveSearch').value.toLowerCase(),room=$('valveRoom').value,pos=$('valvePosition').value;const rows=valveRecords.filter(v=>{const hay=[v.tag,v.plant_room,v.service_duty,v.valve_type,v.size,v.location,v.isolation_purpose,v.notes,v.asset_code].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!room||v.plant_room===room)&&(!pos||v.normal_position===pos)}).sort((a,b)=>String(a.tag).localeCompare(String(b.tag)));
 ...
 body.innerHTML=rows.length?rows.map(v=>`<tr><td><b>${esc(v.tag||'Untagged')}</b><small>${esc(v.asset_code||'No linked asset')}</small></td><td>${esc(v.plant_room||'To confirm')}</td><td>${esc(v.service_duty||'To confirm')}</td><td>${esc([v.valve_type,v.size].filter(Boolean).join(' · ')||'To confirm')}</td><td><span class="opsStatus">${esc(v.normal_position||'To confirm')}</span></td><td>${esc(v.isolation_purpose||'To confirm')}</td><td>${esc(v.last_verified||'Due')}</td><td><button data-valve="${esc(v.id)}">Edit</button></td></tr>`).join(''):'<tr><td colspan="8" class="emptyState">No valves match the filters. Add the first tagged valve above.</td></tr>'}
...
function stopApp() { session=null; currentRole='engineer'; assets=[]; els.appShell.hidden=true; els.authScreen.hidden=false; els.authPassword.value=''; els.authMessage.textContent=''; }
...
els.edit.onclick=()=>{if(requireRole('supervisor','edit assets'))setEditing(!editing)}; els.save.onclick=saveCurrent; els.addAsset.onclick=()=>{if(requireRole('supervisor','add assets'))newAsset()}; els.close.onclick=closeAsset; els.back.onclick=closeAsset; els.modal.onclick=e=>{if(e.target===els.modal)closeAsset()};
...
$('quickAddAsset')?.addEventListener('click',()=>{if(!requireRole('supervisor','add assets'))return;showRegister('');newAsset()});
...
$('valveRows')?.addEventListener('click',e=>{const b=e.target.closest('[data-valve]');if(b)openValve(b.dataset.valve)});
$('importValves')?.addEventListener('click',()=>{if(requireRole('supervisor','import valve registers'))openValveImport()});$('adminAddPlantRoom')?.addEventListener('click',()=>addPlantRoomFromUi(''));$('addImportPlantRoom')?.addEventListener('click',()=>addPlantRoomFromUi('valveImportRoom'));$('addValvePlantRoom')?.addEventListener('click',()=>addPlantRoomFromUi('vRoom'));$('closeValveImport')?.addEventListener('click',()=>closeOpsModal('valveImportModal'));$('cancelValveImport')?.addEventListener('click',()=>closeOpsModal('valveImportModal'));$('valveImportFile')?.addEventListener('change',previewValveImport);$('valveImportRoom')?.addEventListener('change',()=>{if($('valveImportFile').files[0])previewValveImport()});$('confirmValveImport')?.addEventListener('click',confirmValveImport);$('valveImportModal')?.addEventListener('click',e=>{if(e.target.id==='valveImportModal')closeOpsModal('valveImportModal')});
$('addValve')?.addEventListener('click',()=>{if(requireRole('supervisor','add valves'))openValve()});$('saveValve')?.addEventListener('click',saveValveRecord);$('deleteValve')?.addEventListener('click',deleteValveRecord);$('closeValveModal')?.addEventListener('click',()=>closeOpsModal('valveModal'));
