...
function mergeOperationRows(cloudRows=[],localRows=[]){
 ...
 for(const row of [...localRows,...cloudRows]){
  const key=valveIdentity(row)||`row-${map.size}`;
...
async function loadOperations(){
 ...
 ppmRecords=operationsCloudReady?mergeOperationRows(p.data||[],localPpm):localPpm;
 valveRecords=operationsCloudReady?mergeOperationRows(v.data||[],localValves):localValves;
 saveOpsLocal();
 if(operationsCloudReady&&localValves.length){
  const cloudKeys=new Set((v.data||[]).map(valveIdentity));
  const missing=localValves.filter(row=>!cloudKeys.has(valveIdentity(row)));
  if(missing.length){
   const payload=missing.map(row=>{const copy={...row};if(String(copy.id||'').startsWith('local-'))delete copy.id;return copy});
   client.from('valve_register').upsert(payload,{onConflict:'tag'}).then(({data,error})=>{
    if(!error&&Array.isArray(data)&&data.length){valveRecords=mergeOperationRows(data,valveRecords);saveOpsLocal();populateOpsFilters();renderValves()}
   }).catch(()=>{});
  }
 }
