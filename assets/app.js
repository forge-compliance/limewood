...
function renderValveRoomButtons(rooms){
 ...
 grid.innerHTML=rooms.length?rooms.map(name=>{const count=valveRecords.filter(v=>v.plant_room===name).length;return `<button type="button" class="valveRoomCard" data-valve-room-card="${esc(name)}"><span class="roomIcon">🏭</span><span><b>${esc(name.replace(/ Plant Room$/i,''))}</b><small>${count} valve${count===1?'':'s'}</small></span><span class="roomCardArrow">Open register →</span></button>`}).join(''):'<p class="emptyState">No plant rooms have been added yet.</p>';
}
...
function ppmRooms(){
  const rooms=new Set(allKnownPlantRooms());
  ppmRecords.forEach(r=>{const room=r.plant_room||assetForCode(r.asset_code)?.room;if(room)rooms.add(room)});
...
function renderPpmDirectory(){
  ...
  host.innerHTML=rooms.length?rooms.map(room=>{
    const count=ppmRecords.filter(r=>(r.plant_room||assetForCode(r.asset_code)?.room)===room).length;
    return `<button type="button" data-ppm-room="${esc(room)}"><span>🛠</span><b>${esc(room)}</b><small>${count} PPM schedule${count===1?'':'s'}</small></button>`;
...
function showPpmDirectory(){
  ...
  $('ppmPageDescription').textContent='Choose a plant room to open its planned maintenance schedule.';
  renderPpmDirectory();
  closeDrawer();
}
function openPpmRoom(room){
  ...
  $('ppmPageTitle').textContent=`${currentPpmRoom} PPM`;
...
function renderPpm(){const body=$('ppmRows');if(!body)return;const q=$('ppmSearch').value.toLowerCase(),room=$('ppmRoom').value,status=$('ppmStatus').value;const rows=ppmRecords.map(r=>({...r,asset:assetForCode(r.asset_code),computed:ppmStatus(r)})).filter(r=>{const hay=[r.asset_code,r.asset?.name,r.asset?.room,r.assigned_to,r.task,r.notes].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!room||r.asset?.room===room)&&(!status||r.computed===status)}).sort((a,b)=>String(a.next_due||'9999').localeCompare(String(b.next_due||'9999')));
 if($('ppmRoomCount'))$('ppmRoomCount').textContent=`${rows.length} schedule${rows.length===1?'':'s'}`;$('ppmTotal').textContent=rows.length;$('ppmOverdue').textContent=rows.filter(r=>r.computed==='Overdue').length;$('ppmDueSoon').textContent=rows.filter(r=>r.computed==='Due soon').length;$('ppmCompleted').textContent=rows.filter(r=>r.computed==='Complete').length;
...
function renderValves(){const body=$('valveRows');if(!body)return;const room=$('valveRoom').value,rooms=[...new Set([...assets.map(a=>a.room),...plantRooms.map(r=>r.name),...valveRecords.map(v=>v.plant_room),...customPlantRooms()].filter(Boolean))].sort((a,b)=>a.localeCompare(b));
 renderValveRoomButtons(rooms);
 ...
 const rows=filteredValveRows();$('valveRoomTitle').textContent=room;$('valveRoomCount').textContent=`${rows.length} valve${rows.length===1?'':'s'} shown · ${valveRecords.filter(v=>v.plant_room===room).length} registered`;
