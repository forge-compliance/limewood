...
const docEls = {
  ...
  plantRoom:$('dPlantRoom'), asset:$('dAsset'), description:$('dDescription'), changeSummary:$('dChangeSummary'), file:$('dFile'), fileInfo:$('dFileInfo'), progress:$('documentUploadProgress')
};
...
function formatFileSize(bytes){
  ...
  const units=['B','KB','MB','GB']; let value=bytes, i=0;
  while(value>=1024 && i<units.length-1){value/=1024;i++;}
  return `${value.toFixed(i?1:0)} ${units[i]}`;
}
function updateSelectedFileInfo(){
  const file=docEls.file.files[0];
  docEls.fileInfo.textContent=file ? `${file.name} · ${formatFileSize(file.size)}` : 'No file selected';
}

function documentTypeLabel(type) {
  return ({sop:'SOP',rams:'RAMS',manual:'Manual',isolation_procedure:'Isolation procedure',hydraulic_schematic:'Hydraulic schematic',valve_chart:'Valve chart / register',drawing:'Drawing',lighting_schedule:'Lighting schedule',datasheet:'Datasheet',commissioning:'Commissioning sheet',certificate:'Certificate',photo:'Photo',report:'Report',other:'Other'})[type] || type || 'Document';
}
...
function showDocuments(type='') {
  ...
  const labels={sop:'SOP Library',rams:'RAMS Library',manual:'Manufacturer Manuals',hydraulic_schematic:'Hydraulic Schematics',valve_chart:'Valve Charts & Registers',drawing:'Drawings',lighting_schedule:'Lighting Schedules',datasheet:'Datasheets',commissioning:'Commissioning Records',certificate:'Certificates',photo:'Photo Library',report:'Reports'};
...
function openDocumentUpload() {
  ...
  updateDocumentPlantRooms(); docEls.asset.value=''; docEls.description.value=''; docEls.changeSummary.value=''; docEls.file.value=''; docEls.fileInfo.textContent='No file selected'; docEls.progress.hidden=true; docEls.progress.querySelector('span').style.width='0%'; docEls.message.textContent='';
...
async function saveStandardDocument({file,type,title,number,path}) {
  const allowed=['sop','manual','isolation_procedure','rams','hydraulic_schematic','valve_chart','drawing','lighting_schedule','datasheet','commissioning','certificate','photo','report','other'];
...
async function saveLibraryDocument() {
  ...
  try {
    ...
    docEls.save.disabled=true; docEls.save.textContent='Uploading…'; docEls.progress.hidden=false; docEls.progress.querySelector('span').style.width='18%';
    ...
    docEls.progress.querySelector('span').style.width='70%';
    ...
    docEls.progress.querySelector('span').style.width='100%';
    ...
    docEls.save.disabled=false; docEls.save.textContent='Upload to Document Centre'; setTimeout(()=>{ if(docEls.progress) docEls.progress.hidden=true; },500);
  }
}
...
docEls.file.onchange=updateSelectedFileInfo;
