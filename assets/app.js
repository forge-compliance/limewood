...
async function loadOperations(){
 ...
 ppmRecords=operationsCloudReady
  ?mergeOperationRows(p.data||[],localPpm)
  :localPpm;

 valveRecords=operationsCloudReady
  ?mergeOperationRows(v.data||[],localValves)
  :localValves;
 ...
 if(operationsCloudReady&&localValves.length){
  ...
  if(!migration.error&&migration.migrated){
   ...
   if(!refreshed.error){
    valveRecords=mergeOperationRows(refreshed.data||[],localValves);
    saveOpsLocal();
   }
  }
 }
...
const dashboardView=$('dashboardView'),plantRoomHubView=$('plantRoomHubView'),registerView=$('registerView'),documentView=$('documentView'),complianceView=$('complianceView'),ppmView=$('ppmView'),logsView=$('logsView'),valveView=$('valveView'),placeholderView=$('placeholderView'),registerTitle=$('registerTitle'),drawer=$('drawer'),backdrop=$('drawerBackdrop');
let currentHubRoom='';
function closeDrawer(){drawer.classList.remove('open');backdrop.classList.remove('open');drawer.setAttribute('aria-hidden','true')}
function showView(n){dashboardView.hidden=n!=='dashboard';plantRoomHubView.hidden=n!=='plantRoomHub';registerView.hidden=n!=='register';documentView.hidden=n!=='documents';complianceView.hidden=n!=='compliance';ppmView.hidden=n!=='ppm';logsView.hidden=n!=='logs';valveView.hidden=n!=='valves';$('bmsView').hidden=n!=='bms';placeholderView.hidden=n!=='placeholder'}
...
let currentMaintenanceIssue=null;

function issueStatusClass(status){
  const s=String(status||'New').toLowerCase();
  if(s==='new')return 'issueNew';
  if(s==='assigned')return 'issueAssigned';
  if(s==='in progress')return 'issueProgress';
  if(s==='waiting for parts')return 'issueWaiting';
  if(s==='completed'||s==='closed')return 'issueComplete';
  return 'issueNew';
}
function issuePriorityClass(priority){
  const p=String(priority||'Normal').toLowerCase();
  if(p==='urgent')return 'priorityUrgent';
  if(p==='high')return 'priorityHigh';
  if(p==='low')return 'priorityLow';
  return 'priorityNormal';
}
function issueReference(row){
  return row.reference||`MI-${String(row.id||'').slice(-6).toUpperCase()}`;
}
function engineerName(id){
  const e=maintenanceEngineers.find(x=>String(x.id)===String(id));
  return e?.name||e?.email||'Unassigned';
}
async function loadMaintenanceIssues(){
  try{
    const [issuesRes,engRes]=await Promise.all([
      client.from('maintenance_issues').select('*').order('created_at',{ascending:false}),
      client.from('maintenance_engineers').select('*').eq('active',true).order('name')
    ]);
    if(!issuesRes.error)maintenanceIssues=issuesRes.data||[];
    if(!engRes.error)maintenanceEngineers=engRes.data||[];
  }catch(e){console.warn('Maintenance issue load failed',e)}
  populateMaintenanceEngineerFilters();
  renderMaintenanceIssues();
}
function populateMaintenanceEngineerFilters(){
  const opts=maintenanceEngineers.map(e=>`<option value="${esc(e.id)}">${esc(e.name||e.email)}</option>`).join('');
  const filter=$('issueEngineerFilter'); if(filter)filter.innerHTML='<option value="">All engineers</option>'+opts;
  const select=$('maintenanceEngineer'); if(select)select.innerHTML='<option value="">Unassigned</option>'+opts;
}
function renderMaintenanceIssues(){
  const q=String($('issueSearch')?.value||'').toLowerCase();
  const status=$('issueStatusFilter')?.value||'';
  const priority=$('issuePriorityFilter')?.value||'';
  const engineer=$('issueEngineerFilter')?.value||'';

  const rows=maintenanceIssues.filter(r=>{
    const hay=[issueReference(r),r.location,r.category,r.description,r.reporter_name,r.reporter_email].join(' ').toLowerCase();
    return (!q||hay.includes(q))&&(!status||r.status===status)&&(!priority||r.priority===priority)&&(!engineer||String(r.assigned_engineer_id||'')===engineer);
  });

  if($('issueNewCount'))$('issueNewCount').textContent=maintenanceIssues.filter(r=>r.status==='New').length;
  if($('issueAssignedCount'))$('issueAssignedCount').textContent=maintenanceIssues.filter(r=>r.status==='Assigned').length;
  if($('issueProgressCount'))$('issueProgressCount').textContent=maintenanceIssues.filter(r=>r.status==='In Progress').length;
  if($('issueWaitingCount'))$('issueWaitingCount').textContent=maintenanceIssues.filter(r=>r.status==='Waiting for Parts').length;
  if($('issueCompletedCount'))$('issueCompletedCount').textContent=maintenanceIssues.filter(r=>['Completed','Closed'].includes(r.status)).length;

  const host=$('maintenanceIssueCards'); if(!host)return;
  host.innerHTML=rows.length?rows.map(r=>`
    <button class="maintenanceIssueCard" data-issue-id="${esc(r.id)}">
      <div class="issueCardTop">
        <span class="issueRef">${esc(issueReference(r))}</span>
        <span class="issueStatus ${issueStatusClass(r.status)}">${esc(r.status||'New')}</span>
      </div>
      <h3>${esc(r.location||'Location not set')}</h3>
      <p>${esc(r.description||'No description')}</p>
      <div class="issueMeta">
        <span class="${issuePriorityClass(r.priority)}">${esc(r.priority||'Normal')}</span>
        <span>${esc(r.category||'General')}</span>
        <span>${esc(engineerName(r.assigned_engineer_id))}</span>
      </div>
      <small>${esc(new Date(r.created_at).toLocaleString())} · ${esc(r.reporter_name||'Public report')}</small>
    </button>
  `).join(''):'<div class="emptyState">No maintenance issues match this view.</div>';
}
function showMaintenanceIssues(){
  showView('maintenanceIssues');
  loadMaintenanceIssues();
  closeDrawer();
}
function openMaintenanceIssue(id){
  const r=maintenanceIssues.find(x=>String(x.id)===String(id)); if(!r)return;
  currentMaintenanceIssue=r;
  $('maintenanceIssueTitle').textContent=r.description||'Maintenance issue';
  $('maintenanceIssueRef').textContent=issueReference(r);
  $('maintenanceReportedDetails').innerHTML=`
    <div><dt>Location</dt><dd>${esc(r.location||'—')}</dd></div>
    <div><dt>Category</dt><dd>${esc(r.category||'General')}</dd></div>
    <div><dt>Reported by</dt><dd>${esc(r.reporter_name||'Public')}</dd></div>
    <div><dt>Contact</dt><dd>${esc(r.reporter_email||r.reporter_phone||'—')}</dd></div>
    <div><dt>Reported</dt><dd>${esc(new Date(r.created_at).toLocaleString())}</dd></div>
    <div><dt>Photo</dt><dd>${r.photo_url?`<a href="${esc(r.photo_url)}" target="_blank" rel="noopener">Open photo</a>`:'No photo'}</dd></div>`;
  $('maintenanceStatus').value=r.status||'New';
  $('maintenanceEngineer').value=r.assigned_engineer_id||'';
  $('maintenancePriority').value=r.priority||'Normal';
  $('maintenanceEngineerNotes').value=r.engineer_notes||'';
  const history=Array.isArray(r.history)?r.history:[];
  $('maintenanceHistory').innerHTML=history.length?history.map(h=>`
    <div class="maintenanceHistoryItem"><b>${esc(h.date||'')}</b><span>${esc(h.by||'')}</span><p>${esc(h.note||'')}</p></div>
  `).join(''):'<div class="emptyState small">No updates yet.</div>';
  openOpsModal('maintenanceIssueModal');
}
async function saveMaintenanceIssueUpdate(forceComplete=false){
  if(!currentMaintenanceIssue)return;
  const oldStatus=currentMaintenanceIssue.status||'New';
  const newStatus=forceComplete?'Completed':$('maintenanceStatus').value;
  const engineerId=$('maintenanceEngineer').value||null;
  const notes=$('maintenanceEngineerNotes').value.trim();
  const now=new Date().toISOString();
  const who=session?.user?.email||'Engineer';
  const history=Array.isArray(currentMaintenanceIssue.history)?[...currentMaintenanceIssue.history]:[];
  if(newStatus!==oldStatus||notes!==String(currentMaintenanceIssue.engineer_notes||'')){
    history.unshift({date:new Date().toLocaleString(),by:who,note:`${oldStatus} → ${newStatus}${notes?` · ${notes}`:''}`});
  }
  const payload={
    status:newStatus,
    ...
    history,
    updated_at:now,
    completed_at:newStatus==='Completed'?(currentMaintenanceIssue.completed_at||now):currentMaintenanceIssue.completed_at
  };
  const {data,error}=await client.from('maintenance_issues').update(payload).eq('id',currentMaintenanceIssue.id).select().single();
  if(error)return alert(error.message);
  Object.assign(currentMaintenanceIssue,data);
  await notifyMaintenanceAssignment(currentMaintenanceIssue);
  closeOpsModal('maintenanceIssueModal');
  renderMaintenanceIssues();
}
async function notifyMaintenanceAssignment(issue){
  try{
    await fetch('/api/notify-maintenance',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({issue_id:issue.id})
    });
  }catch(e){console.warn('Maintenance notification request failed',e)}
}


$('quickMaintenanceIssues')?.addEventListener('click',showMaintenanceIssues);
['issueSearch','issueStatusFilter','issuePriorityFilter','issueEngineerFilter'].forEach(id=>$(id)?.addEventListener('input',renderMaintenanceIssues));
$('maintenanceIssueCards')?.addEventListener('click',e=>{const b=e.target.closest('[data-issue-id]');if(b)openMaintenanceIssue(b.dataset.issueId)});
$('closeMaintenanceIssueModal')?.addEventListener('click',()=>closeOpsModal('maintenanceIssueModal'));
$('saveMaintenanceIssue')?.addEventListener('click',()=>saveMaintenanceIssueUpdate(false));
$('completeMaintenanceIssue')?.addEventListener('click',()=>saveMaintenanceIssueUpdate(true));
$('newMaintenanceIssue')?.addEventListener('click',()=>window.open('/maintenance-report.html','_blank','noopener'));

const _showViewV71=typeof showView==='function'?showView:null;
if(_showViewV71){
  window.showView=function(name){
    const maintenance=$('maintenanceIssuesView');
    if(maintenance)maintenance.hidden=name!=='maintenanceIssues';
    return _showViewV71(name);
  };
}


/* v7.1.1 Maintenance UI authoritative navigation */
function openMaintenanceDashboardV711(){
  document.querySelectorAll('main > section[id$="View"]').forEach(v=>v.hidden=true);
  const view=document.getElementById('maintenanceIssuesView');
  if(view)view.hidden=false;
  document.getElementById('maintenanceIssueModal')?.setAttribute('aria-hidden','true');
  loadMaintenanceIssues();
  try{closeDrawer()}catch(e){}
  window.scrollTo({top:0,behavior:'smooth'});
}
document.getElementById('quickMaintenanceIssues')?.addEventListener('click',openMaintenanceDashboardV711);
document.getElementById('maintenanceIssueCards')?.addEventListener('click',e=>{
  const b=e.target.closest('[data-issue-id]');
  if(b)openMaintenanceIssue(b.dataset.issueId);
});
['issueSearch','issueStatusFilter','issuePriorityFilter','issueEngineerFilter'].forEach(id=>{
  document.getElementById(id)?.addEventListener('input',renderMaintenanceIssues);
  document.getElementById(id)?.addEventListener('change',renderMaintenanceIssues);
});
document.getElementById('closeMaintenanceIssueModal')?.addEventListener('click',()=>closeOpsModal('maintenanceIssueModal'));
document.getElementById('saveMaintenanceIssue')?.addEventListener('click',()=>saveMaintenanceIssueUpdate(false));
document.getElementById('completeMaintenanceIssue')?.addEventListener('click',()=>saveMaintenanceIssueUpdate(true));
document.getElementById('newMaintenanceIssue')?.addEventListener('click',()=>window.open('/maintenance-report.html','_blank','noopener'));
