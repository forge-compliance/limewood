...
async function loadCloud() {
  ...
  // Preserve the original asset photo object shape so startup behaviour remains unchanged.
  // Signed URLs are generated as before, but asset cards below deliberately do NOT render them.
  const photoUrls = await Promise.all(photos.map(async p => ({
    ...p,
    url: p.storage_path ? await signedUrl(p.storage_path) : ''
  })));
...
function renderGallery() {
  ...
  const items = [
    ...(local.includes('placeholder') ? [] : [{src:local,caption:'Original survey photograph'}]),
    ...cloud.filter(p=>p.url).map(p=>({src:p.url,caption:p.caption || p.photo_type || 'Asset photograph'}))
  ];
